...
namespace NvxEpi.Extensions
{
    public static class SecondaryAudioExtensions
    {
        ...
        public static void SetSecondaryAudioAddress(this ISecondaryAudioStream device, string address)
        {
            ...
            ISecondaryAudioStreamWithHardware deviceWithHardware = device as ISecondaryAudioStreamWithHardware;
            if (deviceWithHardware == null) return;

            Debug.Console(1, device, "Setting Secondary Audio Address : '{0}'", address);

            if (deviceWithHardware.Hardware.DmNaxRouting.DmNaxTransmit.MulticastAddressFeedback.StringValue ==
                address && address != NoRouteString)
            {
                Debug.Console(1, device, "Secondary Audio Address is same as this unit's Tx address: '{0}'", address);
                deviceWithHardware.Hardware.Control.AudioSource =
                    deviceWithHardware.Hardware.Control.DmNaxAudioSourceFeedback;
            }
            else
            {
