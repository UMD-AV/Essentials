    ...
    public class DeviceConfig
    {
        ...
        public void Initialize(string uniqueID, string deviceKey)
        {
            ...
            try
            {
                ...
                List<DeviceConfig> devices = jsonMaster.JsonObject.ToObject<RootObject>().devices;
                DeviceConfig device = devices.FirstOrDefault(d => d.key.Equals(key));
                if (device == null)
                {
            ...
            {
                string msg = string.Format("Device {0} lookup failed:\r{1}", key, e);
        ...
        protected void OnObjectChange(DeviceConfig device, ushort index, ushort type)
        {
            ...
            {
                DeviceChangeEventArgs args = new DeviceChangeEventArgs(device, type);
                args.Index = index;
