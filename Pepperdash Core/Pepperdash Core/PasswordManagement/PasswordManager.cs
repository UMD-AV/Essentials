...
namespace PepperDash.Core.PasswordManagement
{
    public class PasswordManager
    {
        ...
        public void UpdatePassword(ushort key, string password)
        {
            ...
            {
                Debug.Console(1,
                    string.Format("PasswordManager.UpdatePassword: key [{0}] or password are not valid", key,
                        password));
                return;
            }
            ...
            try
            {
                ...
                Debug.Console(1,
                    string.Format("PasswordManager.UpdatePassword: _password[{0}] = {1}", key, _passwords[key]));
            ...
            {
                string msg = string.Format("PasswordManager.UpdatePassword key-value[{0}, {1}] failed:\r{2}", key,
                    password, e);
                Debug.Console(1, msg);
            }
        }
        ...
        private void PasswordTimerElapsed()
        {
            try
            {
                ...
                foreach (KeyValuePair<uint, string> pw in _passwords)
                {
                    // if key exists, continue
                    if (Passwords.ContainsKey(pw.Key))
                    {
                        Debug.Console(1,
                            string.Format("PasswordManager.PasswordTimerElapsed: pw.key[{0}] = {1}", pw.Key, pw.Value));
                        if (Passwords[pw.Key] != _passwords[pw.Key])
                        {
                            Passwords[pw.Key] = _passwords[pw.Key];
                            Debug.Console(1,
                                string.Format("PasswordManager.PasswordTimerElapsed: Updated Password[{0} = {1}",
                                    pw.Key, Passwords[pw.Key]));
                            OnPasswordChange(Passwords[pw.Key], (ushort)pw.Key,
                                PasswordManagementConstants.StringValueChange);
                        }
                    }
                    // else add the key & value
                    else
            ...
            {
                string msg = string.Format("PasswordManager.PasswordTimerElapsed failed:\r{0}", e);
                Debug.Console(1, msg);
            }
        }
        ...
        public void ListPasswords()
        {
            ...
            foreach (KeyValuePair<uint, string> pw in Passwords)
        ...
        {
            EventHandler<UshrtChangeEventArgs> handler = UshrtChange;
            if (handler != null)
            {
        ...
        {
            EventHandler<StringChangeEventArgs> handler = PasswordChange;
            if (handler != null)
            {
                StringChangeEventArgs args = new StringChangeEventArgs(value, type);
                args.Index = index;
