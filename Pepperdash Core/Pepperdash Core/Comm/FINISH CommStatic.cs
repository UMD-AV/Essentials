...
namespace PepperDash.Core
{
    ...
    public static class CommStatic
    {
        ...
        static CommStatic()
        {
            ...
            {
                CrestronConsole.AddNewConsoleCommand(SocketCommand, "socket",
                    "socket commands: list, send, connect, disco",
        ...
        static void SocketCommand(string s)
        {
            ...
            else if (command == "list")
            {
                ...
                int longestLength = Sockets.Aggregate("",
                    (max, cur) => max.Length > cur.Key.Length ? max : cur.Key).Length;
                for (int i = 0; i < Sockets.Count; i++)
                {
                    ISocketStatus sock = Sockets[i];
