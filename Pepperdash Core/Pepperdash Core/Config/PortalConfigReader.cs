    ...
    public class PortalConfigReader
    {
        ...
        public static void ReadAndMergeFileIfNecessary(string filePath, string savePath)
        {
            try
            {
                ...
                {
                    JObject jsonObj = JObject.Parse(fs.ReadToEnd());
                    if (jsonObj["template"] != null && jsonObj["system"] != null)
                    {
                        // it's a double-config, merge it.
                        JObject merged = MergeConfigs(jsonObj);
        ...
        {
            JObject system = JObject.FromObject(doubleConfig["system"]);
            JObject template = JObject.FromObject(doubleConfig["template"]);
        ...
        private static JArray MergeArraysOnTopLevelProperty(JArray a1, JArray a2, string propertyName, string path)
        {
            ...
            {
                if (a2[0]["key"] ==
                    null) // If the first item in the system array has no key, overwrite the template array
                {
                ...
                else // The arrays are keyed, merge them by key
                {
                    for (int i = 0; i < a1.Count(); i++)
                    {
                        ...
                        JToken a2Match =
                            a2.FirstOrDefault(t =>
                                t[propertyName]
                                    .Equals(a1Dev[propertyName])); // t.Value<int>("uid") == tmplDev.Value<int>("uid"));
                        if (a2Match != null)
                        {
                            JObject mergedItem =
                                Merge(a1Dev, a2Match,
                                    string.Format("{0}[{1}].", path,
                                        i)); // Merge(JObject.FromObject(a1Dev), JObject.FromObject(a2Match));
        ...
        private static JObject Merge(JObject o1, JObject o2, string path)
        {
            ...
            {
                string propKey = o2Prop.Key;
                JToken o1Value = o1[propKey];
                JToken o2Value = o2[propKey];
                ...
                {
                    // Drill down
                    string propPath = string.Format("{0}.{1}", path, propKey);
                    try
                    {
                        if (o1Value is JArray)
                        {
                            ...
                            {
                                o1Value.Replace(MergeArraysOnTopLevelProperty(o1Value as JArray, o2Value as JArray,
                                    "key", propPath));
                            }
                        }
                    ...
                    {
                        Debug.Console(1, Debug.ErrorLogLevel.Warning, "Cannot merge items at path {0}: \r{1}", propPath,
                            e);
                    }
                }
            }
