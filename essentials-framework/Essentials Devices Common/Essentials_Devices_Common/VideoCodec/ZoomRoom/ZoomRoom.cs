...
using UmdEssentials.Core.DeviceTypeInterfaces;
...
namespace UmdEssentials.Devices.Common.VideoCodec.ZoomRoom
{
    public class ZoomRoom : VideoCodecBase, IHasCodecSelfView, IHasDirectoryHistoryStack, ICommunicationMonitor,
        IRouting,
        IHasScheduleAwareness, IHasCodecCameras, IHasCameraOff, IHasCameraMuteWithUnmuteReqeust,
        IHasCameraAutoMode,
        ...
        IHasMeetingLock, IHasMeetingRecordingWithPrompt, IHasWaitingRoom, IZoomWirelessShareInstructions
    {
        ...
        private readonly string[] Delimiters = { EchoDelimiter, JsonDelimiter, "OK\x0D\x0A", "end\x0D\x0A" };
            ...
            : base(config)
        {
            ...
            if (_props.CommunicationMonitorProperties != null)
                CommunicationMonitor = new GenericCommunicationMonitor(this, Communication,
                    _props.CommunicationMonitorProperties.PollInterval,
                    _props.CommunicationMonitorProperties.TimeToWarning,
                    _props.CommunicationMonitorProperties.TimeToError,
        ...
        protected override Func<int> VolumeLevelFeedbackFunc
        {
            get
            {
                ...
                {
                    int scaledVol =
                        CrestronEnvironment.ScaleWithLimits(Configuration.Audio.Output.Volume, 100, 0, 65535, 0);

                    if (Configuration.Audio.Output.Volume != 0)
                    {
                        Debug.Console(2, this, "Storing previous volume level as: {0}, scaled: {1}",
                            Configuration.Audio.Output.Volume,
                            scaledVol);
        ...
        protected override Func<bool> PrivacyModeIsOnFeedbackFunc
        {
            get
            {
                return () =>
                {
                    if (IsInCall) return Configuration.Call.Microphone.Mute;
            ...
            get
            {
                return () =>
                {
                    if (Status.Sharing.isAirHostClientConnected)
                        return "Airplay";
                    if (Status.Sharing.isDirectPresentationConnected || Status.Sharing.isBlackMagicConnected)
                        return "Laptop";
                    return "None";
                };
            }
        ...
        protected override Func<bool> SharingContentIsOnFeedbackFunc
        {
            ...
            {
                return () =>
                    Status.Call.Sharing.IsSharing || Status.Sharing.isAirHostClientConnected ||
                    Status.Sharing.isDirectPresentationConnected || Status.Sharing.isSharingBlackMagic;
            }
        }
        ...
        public CameraBase SelectedCamera
        {
            ...
            private set
            {
                ...
                if (handler != null) handler(this, new CameraSelectedEventArgs(_selectedCamera));
            }
        }
        ...
        public void SelectCamera(string key)
        {
            ...
            if (camera != null)
            {
                ...
                if (CameraIsMutedFeedback.BoolValue) CameraMuteOff();
            }
            else
            {
        ...
        {
            CodecDirectory directoryResults = new CodecDirectory();

            directoryResults.AddContactsToDirectory(
                DirectoryRoot.CurrentDirectoryResults.FindAll(c =>
                    c.Name.IndexOf(searchString, 0, StringComparison.OrdinalIgnoreCase) > -1));
        ...
        {
            CodecDirectory directoryResults = new CodecDirectory { ResultsFolderId = folderId };
        ...
        public void GetDirectoryParentFolderContents()
        {
            ...
            CodecDirectory currentDirectory = DirectoryBrowseHistoryStack.Pop();
        ...
        {
            if (a.PropertyName == "meetingIsBeingRecorded" || a.PropertyName == "emailRequired" ||
                a.PropertyName == "canRecord")
            {
                MeetingIsRecordingFeedback.FireUpdate();

                MeetingInfo meetingInfo = new MeetingInfo(MeetingInfo.Id,
                    MeetingInfo.Name,
                    MeetingInfo.Host,
        ...
        private void HandleSharingStateUpdate(object sender, PropertyChangedEventArgs a)
        {
            ...
            try
            {
                ...
                if (MeetingInfo == null)
                {
                    MeetingInfo = new MeetingInfo("", "", "", "", GetSharingStatus(), GetIsHostMyself(),
                        GetIsCohostMyself(), true, false, MeetingIsLockedFeedback.BoolValue,
                        MeetingIsRecordingFeedback.BoolValue, Status.Call.CallRecordInfo.AllowRecord);
                    return;
                }

                MeetingInfo meetingInfo = new MeetingInfo(MeetingInfo.Id, MeetingInfo.Name,
                    Participants.Host != null ? Participants.Host.Name : "None",
                    MeetingInfo.Password, GetSharingStatus(), GetIsHostMyself(), GetIsCohostMyself(),
                    MeetingInfo.IsSharingMeeting, MeetingInfo.WaitingForHost, MeetingIsLockedFeedback.BoolValue,
                    MeetingIsRecordingFeedback.BoolValue, Status.Call.CallRecordInfo.AllowRecord);
                MeetingInfo = meetingInfo;
            }
            catch (Exception e)
            {
                ...
                MeetingInfo = new MeetingInfo("", "", "", "", "None", false, false, false, false,
                    MeetingIsLockedFeedback.BoolValue, MeetingIsRecordingFeedback.BoolValue, false);
            }
        }
        ...
        private void SetUpFeedbackActions()
        {
            ...
            {
                if (a.PropertyName == "Mute") PrivacyModeIsOnFeedback.FireUpdate();
            };

            Configuration.Video.PropertyChanged += (o, a) =>
            {
                if (a.PropertyName == "HideConfSelfVideo") SelfviewIsOnFeedback.FireUpdate();
            };
            Configuration.Video.Camera.PropertyChanged += (o, a) =>
            {
                if (a.PropertyName == "SelectedId") SelectCamera(Configuration.Video.Camera.SelectedId);
        ...
        private void SetUpDirectory()
        {
            ...
            CurrentDirectoryResultIsNotDirectoryRoot =
                new BoolFeedback(() => CurrentDirectoryResult.ResultsFolderId != "root");
        ...
        private void CreateOsdSource()
        {
            ...
            TieLine tl = new TieLine(OsdSource.AudioVideoOutputPort, CodecOsdIn);
        ...
        /// <summary>
        /// Starts the HTTP feedback server and synchronizes the state of the codec
        /// </summary>
        /// <returns></returns>
        ...
        {
            if (CommDebuggingIsOn) Debug.Console(1, this, "Sending: '{0}'", command);
        ...
        private void Port_LineReceived(object dev, GenericCommMethodReceiveTextArgs args)
        {
            ...
            if (_props.DisablePhonebookAutoDownload && args.Text.Substring(7, 9) == "Phonebook") return;
        ...
        private void ResubscribeForAddedContacts()
        {
            ...
            SendText("zFeedback Register Op: in Path: /Event/Phonebook/UpdatedContact");
        }

        /// <summary>
        /// Processes non-JSON responses as they are received
        /// </summary>
        ...
        private void DeserializeResponse(string response)
        {
            try
            {
                ...
                JObject message = JObject.Parse(trimmedResponse);

                if (!_syncState.FirstJsonResponseWasReceived) _syncState.ReceivedFirstJsonResponse();

                eZoomRoomResponseType eType =
                    (eZoomRoomResponseType)
                    Enum.Parse(typeof(eZoomRoomResponseType), message["type"].Value<string>(), true);

                string topKey = message["topKey"].Value<string>();

                JToken responseObj = message[topKey];

                Debug.Console(1, this, "{0} Response Received. topKey: '{1}'\n{2}", eType, topKey,
                    responseObj.ToString().Replace("\n", CrestronEnvironment.NewLine));
                ...
                switch (eType)
                {
                    ...
                    case eZoomRoomResponseType.zCommand:
                    {
                        switch (topKey.ToLower())
                        {
                            ...
                            case "dialjoinresult":
                            {
                                /*var state = message["state"].Value<string>();
                                if (state == "Error")
                                {
                                }*/
                                break;
                            ...
                            case "phonebooklistresult":
                            {
                                ...
                                CodecDirectory directoryResults =
                            ...
                            case "listparticipantsresult":
                            {
                                ...
                                switch (responseObj.Type)
                                {
                                    case JTokenType.Array:
                                        ...
                                        if (responseObj.HasValues)
                                        {
                                            ...
                                            Participants.CurrentParticipants =
                                                zCommand.ListParticipant
                                                    .GetGenericParticipantListFromParticipantsResult(
                                                        Status.Call.Participants);
                                    ...
                                    case JTokenType.Object:
                                    {
                                        ...
                                        zCommand.ListParticipant participant =
                                        ...
                                        if (participant != null)
                                        {
                                            ...
                                            switch (participant.Event)
                                            {
                                                ...
                                                {
                                                    int index = Status.Call.Participants.FindIndex(p =>
                                                        p.UserId.Equals(participant.UserId));
                                                            ...
                                                            case "ZRCUserChangedEventLeftMeeting":
                                                                ...
                                                                Participants.CurrentParticipants =
                                                                    zCommand.ListParticipant
                                                                        .GetGenericParticipantListFromParticipantsResult(
                                                                            Status.Call.Participants);
                                                                Participants.OnParticipantRemoved();
                                                                break;
                                                            case "ZRCUserChangedEventUserInfoUpdated":
                                                                ...
                                                                int sortedIndex =
                                                                    Participants.CurrentParticipants.FindIndex(p =>
                                                                        p.UserId.Equals(participant.UserId));
                                                                if (Participants.CurrentParticipants[sortedIndex]
                                                                        .HandIsRaisedFb != participant.HandStatus
                                                                        .HandIsRaisedAndValid)
                                                                {
                                                                ...
                                                                else
                                                                {
                                                                    ...
                                                                    Participants.OnParticipantUpdated(sortedIndex,
                                                                        zCommand.ListParticipant
                                                                            .GetGenericParticipantFromParticipantResult(
                                                                                existingParticipant));
                                                                }
                    ...
                    case eZoomRoomResponseType.zEvent:
                    {
                        switch (topKey.ToLower())
                        {
                            case "phonebook":
                            {
                                ...
                                {
                                    JToken jToken = responseObj["Updated Contact"];
                                    if (jToken != null)
                                    {
                                        zStatus.Contact newContact =
                                            JsonConvert.DeserializeObject<zStatus.Contact>(
                                                jToken.ToString());
                            ...
                            {
                                if (!_syncState.InitialSyncComplete) _syncState.LastQueryResponseReceived();
                            ...
                            {
                                zEvent.IncomingCallIndication incomingCall =
                                    JsonConvert.DeserializeObject<zEvent.IncomingCallIndication>(
                                        responseObj.ToString());

                                if (incomingCall != null)
                                {
                                    CodecActiveCallItem newCall = new CodecActiveCallItem
                                    {
                                    ...
                                    if (ActiveCalls.Count > 0)
                                    {
                                        ...
                                        if (activeCall != null)
                                        {
                                            Debug.Console(1, this,
                                                ...
                                                ActiveCalls.Count, activeCall.Id, activeCall.Number, activeCall.Name,
                            ...
                            case "callconnecterror":
                            {
                                ...
                                EventHandler<StringChangeEventArgs> handler = CallConnectError;
                                if (handler != null) handler(this, new StringChangeEventArgs(errorMessage, 0));

                                break;
                            }
                            case "videounmuterequest":
                            {
                                EventHandler handler = VideoUnmuteRequested;
                            ...
                            {
                                zEvent.MeetingNeedsPassword meetingNeedsPassword =
                                    responseObj.ToObject<zEvent.MeetingNeedsPassword>();

                                if (meetingNeedsPassword.NeedsPassword)
                                {
                                    const string prompt =
                                        "Password required to join this meeting.  Please enter the meeting password.";
                            ...
                            case "needwaitforhost":
                            {
                                ...
                                {
                                    MeetingInfo = new MeetingInfo("Waiting For Host", "Waiting For Host",
                                        "Waiting For Host", "",
                                        GetSharingStatus(), false, false, false, true,
                                        MeetingIsLockedFeedback.BoolValue, MeetingIsRecordingFeedback.BoolValue,
                                        Status.Call.CallRecordInfo.AllowRecord);
                                ...
                                if (MeetingInfo == null)
                                {
                                    MeetingInfo = new MeetingInfo("Waiting For Host", "Waiting For Host",
                                        "Waiting For Host", "",
                                        GetSharingStatus(), false, false, false, false,
                                        MeetingIsLockedFeedback.BoolValue, MeetingIsRecordingFeedback.BoolValue,
                                        Status.Call.CallRecordInfo.AllowRecord);
                                    break;
                                }

                                MeetingInfo = new MeetingInfo(MeetingInfo.Id, MeetingInfo.Name, MeetingInfo.Host,
                                    MeetingInfo.Password,
                                    GetSharingStatus(), GetIsHostMyself(), GetIsCohostMyself(), false, false,
                                    MeetingIsLockedFeedback.BoolValue, MeetingIsRecordingFeedback.BoolValue,
                                    Status.Call.CallRecordInfo.AllowRecord);
                            ...
                            case "pinstatusofscreennotification":
                            {
                                ...
                                if (status.PinnedUserId > 0)
                                {
                                    alreadyPinnedParticipant =
                                        Participants.CurrentParticipants.FirstOrDefault(p =>
                                            p.ScreenIndexIsPinnedToFb.Equals(status.ScreenIndex));

                                    // Make sure that the already pinned participant is different ID as for this message.
                                    // If true, clear the pinned fb.
                                    if (alreadyPinnedParticipant != null &&
                                        alreadyPinnedParticipant.UserId != status.PinnedUserId)
                                    {
                                        Debug.Console(1, this,
                                            "Participant: {0} with id: {1} already pinned to screenIndex {2}.  Clearing pinned fb.",
                                ...
                                Participant participant =
                                    Participants.CurrentParticipants.FirstOrDefault(p =>
                                        p.UserId.Equals(status.PinnedUserId));
                                ...
                                else
                                {
                                    ...
                                    {
                                        Debug.Console(1, this,
                                            "no matching participant found by pinned_user_id: {0} or screen_index: {1}",
                                    ...
                                    if (participant != null)
                                    {
                                        Debug.Console(2, this, "Unpinning {0} with id: {1} from screen index: {2}",
                                            participant.Name,
                                ...
                                // fire the event as we've modified the participant list
                                Participants.OnParticipantsChanged();
                            ...
                            {
                                zEvent.StartLocalPresentMeeting result =
                                    JsonConvert.DeserializeObject<zEvent.StartLocalPresentMeeting>(
                                        responseObj.ToString());

                                if (result.Success)
                                    MeetingInfo = new MeetingInfo("", "", "", "", GetSharingStatus(), true, false, true,
                                        MeetingInfo.WaitingForHost, MeetingIsLockedFeedback.BoolValue,
                                        MeetingIsRecordingFeedback.BoolValue, Status.Call.CallRecordInfo.AllowRecord);
                    ...
                    case eZoomRoomResponseType.zStatus:
                    {
                        switch (topKey.ToLower())
                        {
                            ...
                            case "call":
                            {
                                ...
                                foreach (zCommand.ListParticipant participant in Status.Call.Participants)
                                    Debug.Console(1, this,
                            ...
                            case "video camera line":
                            {
                                ...
                                if (!_syncState.CamerasHaveBeenSetUp) SetUpCameras();
            ...
            catch (Exception ex)
            {
                ...
                {
                    Debug.Console(1, this, "Error Deserializing feedback inner exception: {0}",
            ...
            else
            {
                if (_props.DefaultCallLayout == (_props.DefaultCallLayout & AvailableLayouts))
                    ...
                    Debug.Console(0, this,
                        "Unable to set default Layout.  {0} not currently an available layout based on meeting state",
                        _props.DefaultCallLayout);
            }
        }
        ...
        /// <summary>
        /// Prints the current call participants list
        /// </summary>
        public void PrintCurrentCallParticipants()
        {
            ...
            foreach (Participant participant in Participants.CurrentParticipants)
        ...
        private void UpdateCallStatus()
        {
            Debug.Console(1, this,
                ...
                Status.NeedWaitForHost.Wait);

            if (Status.Call != null)
            {
                zStatus.eCallStatus callStatus = Status.Call.Status;

                // If not currently in a meeting, initialize the call object
                if (callStatus != zStatus.eCallStatus.IN_MEETING &&
                    callStatus != zStatus.eCallStatus.CONNECTING_MEETING)
                {
                ...
                else
                {
                    ...
                    {
                        case zStatus.eCallStatus.IN_MEETING:
                            if (Status.NeedWaitForHost.Wait) Status.NeedWaitForHost.Wait = false;

                            if (existingCall != null) existingCall.Status = eCodecCallStatus.Connected;
                        ...
                        case zStatus.eCallStatus.NOT_IN_MEETING:
                            ...
                            MeetingIsRecordingFeedback.FireUpdate();
                            if (existingCall != null) existingCall.Status = eCodecCallStatus.Disconnected;
                            break;
                    }

                    Debug.Console(1, this,
                        "[UpdateCallStatus] ELSE ActiveCalls.Count == {1} - Current Call Status: {0}",
                        Status.Call != null ? Status.Call.Status.ToString() : "no call", ActiveCalls.Count);
            ...
            for (int i = 0; i < ActiveCalls.Count; i++)
            {
                ...
                {
                    Debug.Console(1, this, "[UpdateCallStatus] Removing Inactive call.Id: {1} call.Name: {0}",
                        call.Name, call.Id);
            ...
            if (ActiveCalls.Count == 0)
            {
                ...
                List<Participant> emptyList = new List<Participant>();
        ...
        private string GetSharingStatus()
        {
            ...
            try
            {
                if (Status.Call.Sharing.State == zEvent.eSharingState.Receiving) sharingState = "Receiving Content";

                if (Status.Sharing.isAirHostClientConnected) sharingState = "Sharing AirPlay";

                if (Status.Sharing.isDirectPresentationConnected) sharingState = "Sharing Laptop";

                if (Status.Sharing.isSharingBlackMagic) sharingState = "Sharing HDMI Source";
        ...
        private bool GetIsHostMyself()
        {
            try
            {
                ...
                Participant host = Participants.Host;
        ...
        private bool GetIsCohostMyself()
        {
            try
            {
                ...
                Participant cohost = Participants.CurrentParticipants.FirstOrDefault(p => p.IsCohost && p.IsMyself);
        ...
        /// <summary>
        /// Starts sharing the HDMI source
        /// </summary>
        ...
        {
            // TODO: Implement volume decrement that calls SetVolume()
        }
        ...
        {
            int scaledLevel = CrestronEnvironment.ScaleWithLimits(level, 65535, 0, 100, 0);
        ...
        {
            // No corresponding function on the device
        }
        ...
        {
            ZoomRoomJoinMap joinMap = new ZoomRoomJoinMap(joinStart);
        ...
        {
            IHasMeetingInfo meetingInfoCodec = this;
            ...
            IHasMeetingRecordingWithPrompt recordingCodec = this;
            ...
            recordingCodec.MeetingIsRecordingFeedback.LinkInputSig(
                trilist.BooleanInput[joinMap.StartRecording.JoinNumber]);
            recordingCodec.MeetingIsRecordingFeedback.LinkComplementInputSig(
                trilist.BooleanInput[joinMap.StopRecording.JoinNumber]);

            trilist.SetSigFalseAction(joinMap.RecordingPromptAgree.JoinNumber,
                () => recordingCodec.RecordingPromptAcknowledgement(true));
            trilist.SetSigFalseAction(joinMap.RecordingPromptDisagree.JoinNumber,
                () => recordingCodec.RecordingPromptAcknowledgement(false));

            recordingCodec.RecordConsentPromptIsVisible.LinkInputSig(
                trilist.BooleanInput[joinMap.RecordConsentPromptIsVisible.JoinNumber]);

            IHasZoomRoomLayouts layoutsCodec = this;
            layoutsCodec.LayoutInfoChanged += (o, a) =>
            {
                ...
                trilist.SetString(joinMap.LayoutGalleryIsAvailable.JoinNumber,
                    zConfiguration.eLayoutStyle.Gallery.ToString());
                trilist.SetString(joinMap.LayoutSpeakerIsAvailable.JoinNumber,
                    zConfiguration.eLayoutStyle.Speaker.ToString());
                trilist.SetString(joinMap.LayoutStripIsAvailable.JoinNumber,
                    zConfiguration.eLayoutStyle.Strip.ToString());
                trilist.SetString(joinMap.LayoutShareAllIsAvailable.JoinNumber,
                    zConfiguration.eLayoutStyle.ShareAll.ToString());
            };

            trilist.SetSigFalseAction(joinMap.SwapContentWithThumbnail.JoinNumber,
                () => layoutsCodec.SwapContentWithThumbnail());
            ...
            layoutsCodec.LayoutViewIsOnLastPageFeedback.LinkInputSig(
                trilist.BooleanInput[joinMap.LayoutIsOnLastPage.JoinNumber]);
            trilist.SetSigFalseAction(joinMap.LayoutTurnToNextPage.JoinNumber,
                () => layoutsCodec.LayoutTurnNextPage());
            trilist.SetSigFalseAction(joinMap.LayoutTurnToPreviousPage.JoinNumber,
                () => layoutsCodec.LayoutTurnPreviousPage());
            trilist.SetSigFalseAction(joinMap.GetAvailableLayouts.JoinNumber,
                () => layoutsCodec.GetAvailableLayouts());
            ...
            layoutsCodec.LocalLayoutFeedback.LinkInputSig(
                trilist.StringInput[joinMap.GetSetCurrentLayout.JoinNumber]);

            IHasParticipantPinUnpin pinCodec = this;
            ...
            trilist.SetUShortSigAction(joinMap.ScreenIndexToPinUserTo.JoinNumber,
                (u) => ScreenIndexToPinUserTo = u);

            IHasSelfviewSize layoutSizeCodec = this;
            trilist.SetSigFalseAction(joinMap.GetSetSelfviewPipSize.JoinNumber,
                layoutSizeCodec.SelfviewPipSizeToggle);
            ...
            layoutSizeCodec.SelfviewPipSizeFeedback.LinkInputSig(
                trilist.StringInput[joinMap.GetSetSelfviewPipSize.JoinNumber]);

            WaitingRoomChanged += (o, a) => { trilist.SetBool(joinMap.InWaitingRoom.JoinNumber, a.InWaitingRoom); };
            ...
            trilist.SetSigTrueAction(joinMap.StartNormalMeetingFromSharingOnlyMeeting.JoinNumber,
                StartNormalMeetingFromSharingOnlyMeeting);
            ...
            PasswordRequired += (devices, args) =>
            {
                ...
                trilist.SetBool(joinMap.PasswordPromptMessage.JoinNumber, !args.LoginAttemptCancelled);
            };
            ...
            _errorMessageTimer = new CTimer(o => { trilist.SetString(joinMap.CallConnectErrorMessage.JoinNumber, ""); },
                Timeout.Infinite);
            ...
            IZoomWirelessShareInstructions wirelessInfoCodec = this;
            if (Status != null && Status.Sharing != null) SetSharingStateJoins(Status.Sharing, trilist, joinMap);
        ...
        private void SetSharingStateJoins(zStatus.Sharing state, BasicTriList trilist, ZoomRoomJoinMap joinMap)
        {
            ...
            trilist.SetBool(joinMap.IsSharingHdmi.JoinNumber,
                state.isBlackMagicConnected || state.isDirectPresentationConnected);
        ...
        {
            CodecActiveCallItem incomingCall =
                ActiveCalls.FirstOrDefault(c =>
                    c.Status.Equals(eCodecCallStatus.Ringing) && c.Direction.Equals(eCodecCallDirection.Incoming));
        ...
        {
            InvitableDirectoryContact ic = contact as InvitableDirectoryContact;
        ...
        {
            if (duration == 0) duration = DefaultMeetingDurationMin;
            ...
            // Add each invitee
            foreach (InvitableDirectoryContact contact in contacts)
            {
                string invitee = string.Format(" user: {0}", contact.ContactId);
        ...
        public void InviteContactsToExistingMeeting(List<InvitableDirectoryContact> contacts)
        {
            ...
            // Add the prefix
            message.Append("zCommand Call Invite");
        ...
        private void OnDirectoryResultReturned(CodecDirectory result)
        {
            try
            {
                ...
                CodecDirectory directoryResult = result;
                bool directoryIsRoot = CurrentDirectoryResultIsNotDirectoryRoot.BoolValue == false;

                // If the result is Root, create a copy and filter out contacts whose parent folder is not root
                ...
                // This will return the latest results to all UIs.
                // Multiple independent UI Directory browsing will require a different methodology
        ...
        /// <summary>
        /// Builds the camera list by using the Zoom Room zStatus.Cameras data.
        /// Could later be modified to build from config data
        /// </summary>
        private void SetUpCameras()
        {
            ...
            foreach (zStatus.AudioVideoInputOutputLineItem cam in Status.Cameras)
            {
                // Known Issue:
                // Crestron UC engine systems seem
                // to report an item in the camera list that represents the USB bridge device. 
                // If we know the name, and it's reliably consistent, we could ignore it here...

                if (cam.Name.IndexOf("HD-CONV-USB", StringComparison.Ordinal) > -1)
                    // Skip this as it's the Crestron USB box, not a real camera
                    continue;

                CameraBase existingCam = Cameras.FirstOrDefault((c) => c.Key.Equals(cam.id));

                if (existingCam == null)
                {
                    ZoomRoomCamera camera = new ZoomRoomCamera(cam.id, cam.Name, this);

                    Cameras.Add(camera);

                    if (cam.Selected) SelectedCamera = camera;
                }
            }

            if (IsInCall) UpdateFarEndCameras();
        ...
        public void AdmitParticipantFromWaitingRoomIndex(ushort index)
        {
            ...
            {
                int userId = Participants.CurrentParticipants[index].UserId;
        ...
        {
            SendText("zCommand Call MuteAll Mute: on");
        }
        ...
        public void ToggleAudioForParticipantIndex(ushort index)
        {
            if (Participants.CurrentParticipants.Count > index)
            {
                Participant user = Participants.CurrentParticipants[index];
                int userId = Participants.CurrentParticipants[index].UserId;
        ...
        {
            Participant user = Participants.CurrentParticipants.FirstOrDefault(p => p.UserId.Equals(userId));
        ...
        //Zoom doesn't support camera auto modes. Setting this to unmute video
        ...
        public void SelfviewPipPositionToggle()
        {
            ...
            {
                int nextPipPositionIndex = SelfviewPipPositions.IndexOf(_currentSelfviewPipPosition) + 1;
        ...
        public readonly List<CodecCommandWithLabel> SelfviewPipPositions = new List<CodecCommandWithLabel>
        {
        ...
        private void ComputeSelfviewPipPositionStatus()
        {
            _currentSelfviewPipPosition =
                SelfviewPipPositions.FirstOrDefault(p =>
                    p.Command.ToLower().Equals(Configuration.Call.Layout.Position.ToString().ToLower()));
        }
        ...
        public void SelfviewPipSizeToggle()
        {
            ...
            {
                int nextPipSizeIndex = SelfviewPipSizes.IndexOf(_currentSelfviewPipSize) + 1;
        ...
        public readonly List<CodecCommandWithLabel> SelfviewPipSizes = new List<CodecCommandWithLabel>
        {
        ...
        private void ComputeSelfviewPipSizeStatus()
        {
            _currentSelfviewPipSize =
                SelfviewPipSizes.FirstOrDefault(p =>
                    p.Command.ToLower().Equals(Configuration.Call.Layout.Size.ToString().ToLower()));
        }
        ...
        /// <summary>
        /// Reads individual properties to determine if layouts are available
        /// </summary>
        private void ComputeAvailableLayouts()
        {
            ...
            {
                if (Status.Layout.can_Switch_Wall_View) availableLayouts |= zConfiguration.eLayoutStyle.Gallery;

                if (Status.Layout.can_Switch_Speaker_View) availableLayouts |= zConfiguration.eLayoutStyle.Speaker;

                if (Status.Layout.can_Switch_Share_On_All_Screens)
                    availableLayouts |= zConfiguration.eLayoutStyle.ShareAll;

                // There is no property that directly reports if strip mode is valid, but the API stipulates
                // that strip mode is available if the number of screens is 1
                if (Status.NumberOfScreens != null && Status.Layout.video_type != null)
                    if (Status.NumberOfScreens.NumOfScreens == 1 || Status.Layout.can_Switch_Strip_View ||
                        Status.Layout.video_type.ToLower() == "strip")
        ...
        {
            EventHandler<LayoutInfoChangedEventArgs> handler = LayoutInfoChanged;
            if (handler != null)
            {
                zConfiguration.eLayoutStyle currentLayout = (zConfiguration.eLayoutStyle)Enum.Parse(
                    typeof(zConfiguration.eLayoutStyle),
                    string.IsNullOrEmpty(LocalLayoutFeedback.StringValue) ? "None" : LocalLayoutFeedback.StringValue,
                    true);
        ...
        public void SwapContentWithThumbnail()
        {
            ...
            {
                string oppositeValue = ContentSwappedWithThumbnailFeedback.BoolValue ? "on" : "off";
        ...
        {
            string currentLayout = LocalLayoutFeedback.StringValue;

            int eCurrentLayout = (int)Enum.Parse(typeof(zConfiguration.eLayoutStyle), currentLayout, true);

            zConfiguration.eLayoutStyle nextLayout = GetNextLayout(eCurrentLayout);

            if (nextLayout != zConfiguration.eLayoutStyle.None) SetLayout(nextLayout);
        }
        ...
        {
            if (AvailableLayouts == zConfiguration.eLayoutStyle.None) return zConfiguration.eLayoutStyle.None;
            ...
            if ((AvailableLayouts & nextLayout) == nextLayout) return nextLayout;
        ...
        public MeetingInfo MeetingInfo
        {
            ...
            private set
            {
                if (value != _meetingInfo)
                {
                    ...
                    EventHandler<MeetingInfoEventArgs> handler = MeetingInfoChanged;
                    if (handler != null) handler(this, new MeetingInfoEventArgs(_meetingInfo));
                }
            }
        }
        ...
        {
            SendText(string.Format("zCommand Dial Sharing Duration: {0} DisplayState: {1} Password: {2}", duration,
        ...
        {
            SendText("zConfiguration Call Lock Enable: on");
        }

        public void UnLockMeeting()
        {
            SendText("zConfiguration Call Lock Enable: off");
        }
        ...
        {
            string command = string.Format("zCommand Agree Recording: {0}", agree ? "on" : "off");
        ...
        {
            SendText("zCommand Call Record Enable: on");
        }

        public void StopRecording()
        {
            SendText("zCommand Call Record Enable: off");
        }
        ...
        #region IHasWaitingRoom Members

        public event EventHandler<WaitingRoomEventArgs> WaitingRoomChanged;

        private bool _inWaitingRoom;

        public bool InWaitingRoom
        {
            get { return _inWaitingRoom; }
            private set
            {
                if (value != _inWaitingRoom)
                {
                    _inWaitingRoom = value;

                    EventHandler<WaitingRoomEventArgs> handler = WaitingRoomChanged;
                    if (handler != null) handler(this, new WaitingRoomEventArgs(_inWaitingRoom));
                }
            }
        }
...
",
            ...
            EventHandler<ShareInfoEventArgs> handler = ShareInfoChanged;
            if (handler != null) handler(this, new ShareInfoEventArgs(status));
        }
    ...
    public class ZoomRoomInfo : VideoCodecInfo
    {
        ...
        [JsonIgnore] public ZoomRoomStatus Status { get; private set; }
        [JsonIgnore] public ZoomRoomConfiguration Configuration { get; private set; }
        ...
        public override string E164Alias
        {
            get
            {
                if (!string.IsNullOrEmpty(Status.SystemUnit.MeetingNumber)) return Status.SystemUnit.MeetingNumber;
        ...
        public override string SipPhoneNumber
        {
            get
            {
                if (!string.IsNullOrEmpty(Status.Call.Info.dialIn)) return Status.Call.Info.dialIn;
    ...
    /// <summary>
    /// Tracks the initial synchronization state when establishing a new connection
    /// </summary>
    public class ZoomRoomSyncState : IKeyed
    {
        ...
        private void DequeueQueries()
        {
            ...
            {
                string query = _syncQueries.Dequeue();
        ...
        public void ReceivedFirstJsonResponse()
        {
            ...
            EventHandler handler = FirstJsonResponseReceived;
            if (handler != null) handler(this, null);
        ...
        {
            if (LoginResponseWasReceived && FirstJsonResponseWasReceived && InitialQueryMessagesWereSent &&
                LastQueryResponseWasReceived &&
