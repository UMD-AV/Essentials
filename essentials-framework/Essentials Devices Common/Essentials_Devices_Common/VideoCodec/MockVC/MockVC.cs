        ...
        IHasDirectory, IHasCodecCameras, IHasCameraAutoMode, IHasCodecRoomPresets
    {
            ...
            : base(config)
        {
            ...
            CodecOsdIn = new RoutingInputPort(RoutingPortNames.CodecOsd,
                eRoutingSignalType.Audio | eRoutingSignalType.Video, eRoutingPortConnectionType.Hdmi, 0, this);
            InputPorts.Add(CodecOsdIn);
            HdmiIn1 = new RoutingInputPort(RoutingPortNames.HdmiIn1,
            ...
            HdmiIn2 = new RoutingInputPort(RoutingPortNames.HdmiIn2,
                eRoutingSignalType.Audio | eRoutingSignalType.Video, eRoutingPortConnectionType.Hdmi, 2, this);
            InputPorts.Add(HdmiIn2);
            HdmiOut = new RoutingOutputPort(RoutingPortNames.HdmiOut,
                eRoutingSignalType.Audio | eRoutingSignalType.Video, eRoutingPortConnectionType.Hdmi, null, this);
            OutputPorts.Add(HdmiOut);

            CallHistory = new CodecCallHistory();
            for (int i = 0; i < 10; i++)
            {
                CodecCallHistory.CallHistoryEntry call = new CodecCallHistory.CallHistoryEntry();
                call.Name = "Call " + i;
        ...
        public CodecScheduleAwareness CodecSchedule
        {
            get
            {
                ...
                if (_CodecSchedule == null || _CodecSchedule.Meetings.Count == 0
                                           || _CodecSchedule.Meetings[_CodecSchedule.Meetings.Count - 1].StartTime <
                                           DateTime.Now)
                {
        ...
        {
            get { return MockVideoCodecDirectory.DirectoryRoot; }
        }
        ...
        public CodecPhonebookSyncState PhonebookSyncState
        {
            get
            {
                CodecPhonebookSyncState syncState = new CodecPhonebookSyncState(Key + "PhonebookSync");
        ...
        {
            CodecDirectory searchResults = new CodecDirectory();
        ...
        {
            CodecDirectory folderDirectory = new CodecDirectory();
        ...
        private void SetupCameras()
        {
            ...
            MockVCCamera internalCamera = new MockVCCamera(Key + "-camera1", "Near End", this);

            Cameras.Add(internalCamera);

            MockFarEndVCCamera farEndCamera = new MockFarEndVCCamera(Key + "-cameraFar", "Far End", this);
        ...
        {
            CameraBase camera = Cameras.FirstOrDefault(c => c.Key.ToLower().IndexOf(key.ToLower()) > -1);
            if (camera != null)
            {
        ...
        {
            CodecRoomPreset editPreset = NearEndPresets.FirstOrDefault(p => p.ID.Equals(preset));
