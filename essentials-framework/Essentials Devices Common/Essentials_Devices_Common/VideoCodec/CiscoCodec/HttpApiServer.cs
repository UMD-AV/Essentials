    ...
    public class HttpApiServer
    {
        ...
        void Server_Request(object sender, OnHttpRequestArgs args)
        {
            if (args.Request.Header.RequestType == "OPTIONS")
            {
                ...
                args.Response.Header.SetHeaderValue("Access-Control-Allow-Methods",
                    "GET, POST, PATCH, PUT, DELETE, OPTIONS");
                return;
            }

            string path = Uri.UnescapeDataString(args.Request.Path);
            string host = args.Request.DataConnection.RemoteEndPointAddress;
            //string authToken;
            ...
            {
                EventHandler<OnHttpRequestArgs> handler = ApiRequest;
                if (ApiRequest != null)
