    ...
    public class CiscoCodecBookings
    {
        ...
        public static List<Meeting> GetGenericMeetingsFromBookingResult(List<Booking> bookings)
        {
            ...
            foreach (Booking b in bookings)
            {
                ...
                if (b.Organizer != null)
                    meeting.Organizer =
                        string.Format("{0}, {1}", b.Organizer.LastName.Value, b.Organizer.FirstName.Value);
                if (b.Title != null)
...
//#warning Update this ConnectMode conversion after testing onsite.  Expected value is "OBTP", but in PD NYC Test scenarios, "Manual" is being returned for OBTP meetings
                if (b.DialInfo.ConnectMode != null)
                    if (b.DialInfo.ConnectMode.Value.ToLower() == "obtp" ||
                        b.DialInfo.ConnectMode.Value.ToLower() == "manual")
                ...
                if (Debug.Level > 0)
                {
                    Debug.Console(1, "Title: {0}, ID: {1}, Organizer: {2}, Agenda: {3}", meeting.Title, meeting.Id,
                        meeting.Organizer, meeting.Agenda);
            ...
            int joinableCooldownSeconds)
        {
            ...
            foreach (Booking b in bookings)
            {
                Meeting meeting = new Meeting(joinableCooldownSeconds);
