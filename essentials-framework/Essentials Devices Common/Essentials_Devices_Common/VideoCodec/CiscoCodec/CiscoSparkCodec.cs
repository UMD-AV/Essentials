...
namespace UmdEssentials.Devices.Common.VideoCodec.Cisco
{
        ...
        IHasHalfWakeMode, IHasCallHold, IJoinCalls
    {
        ...
        {
            get { return () => _currentLocalLayout.Label; }
        }

        protected Func<bool> LocalLayoutIsProminentFeedbackFunc
        {
            get { return () => _currentLocalLayout.Label == "Prominent"; }
        }
            ...
            : base(config)
        {
            ...
            if (props.PhonebookResultsLimit > 0) _phonebookResultsLimit = props.PhonebookResultsLimit;
            ...
            {
                string command = string.Format("xCommand Peripherals HeartBeat ID: {0}{1}",
                    CrestronEthernetHelper.GetEthernetParameter(
                        CrestronEthernetHelper.ETHERNET_PARAMETER_TO_GET.GET_MAC_ADDRESS, 0), Delimiter);
                CommunicationMonitor =
                    new GenericCommunicationMonitor(this, Communication, 30000, 120000, 300000, command);
            }
            ...
            HdmiIn2 = new RoutingInputPort(RoutingPortNames.HdmiIn2,
                eRoutingSignalType.Audio | eRoutingSignalType.Video,
                eRoutingPortConnectionType.Hdmi, new Action(SelectPresentationSource1), this);
            HdmiIn3 = new RoutingInputPort(RoutingPortNames.HdmiIn3,
                eRoutingSignalType.Audio | eRoutingSignalType.Video,
            ...
            if (props.UiBranding == null) return;
        ...
        private void SetFeedbackActions()
        {
            ...
            {
                SupportsCameraAutoMode = CodecStatus.Status.Cameras.SpeakerTrack.Availability.BoolValue;
            };
            ...
            catch (Exception ex)
            {
                ...
                if (ex.InnerException != null) Debug.Console(0, this, "Error setting MainVideuMute Action: {0}", ex);
            }
        }
        ...
        public override void Initialize()
        {
            ...
            _cliFeedbackRegistrationExpression =
                ...
                prefix + "/Event/CallDisconnect" +
                Delimiter; // Keep CallDisconnect last to detect when feedback registration completes correctly
        }
        ...
        private void SyncState_InitialSyncCompleted(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    foreach (CiscoCodecStatus.Camera camera in CodecStatus.Status.Cameras.Camera)
                    {
                        Debug.Console(0, this,
                            @"Camera id: {0}
Name: {1}
ConnectorID: {2}"
                        ...
                        ushort id = Convert.ToUInt16(camera.id);
            ...
            PhonebookRefreshTimer =
                new CTimer(CheckCurrentHour, 3600000,
                    3600000); // check each hour to see if the phonebook should be downloaded
            GetPhonebook(null);

            BookingsRefreshTimer =
                new CTimer(GetBookings, 900000, 900000); // 15 minute timer to check for new booking info
            GetBookings(null);
        ...
        {
            EnqueueCommand(string.Format("xCommand Peripherals Connect ID:{0} Name:Crestron Type:ControlSystem",
                CrestronEthernetHelper.GetEthernetParameter(
        ...
        private void DeserializeResponse(string response)
        {
            try
            {
                ...
                if (response.IndexOf("\"Status\":{") > -1 || response.IndexOf("\"Status\": {") > -1)
                {
                    ...
                    CiscoCodecStatus.Conference2 conference = tempCodecStatus.Status.Conference;
                    ...
                    if (conference.Presentation.LocalInstance.Count > 0)
                    {
                        ...
                        else if (conference.Presentation.LocalInstance[0].Source != null)
                        {
                            ...
                            if (conference.Presentation.LocalInstance.Any((i) =>
                                    !string.IsNullOrEmpty(i.SendingMode.Value)))
                            {
                    ...
                    if (tempCodecStatus.Status.Call.Count > 0)
                        ...
                        {
                            CodecActiveCallItem tempActiveCall = ActiveCalls.FirstOrDefault(c => c.Id.Equals(call.id));
                            ...
                            else if (call.ghost == null) // if the ghost value is present the call has ended already
                            {
                                ...
                                CodecActiveCallItem newCallItem = new CodecActiveCallItem
                                {
                                    Id = call.id,
                    ...
                    List<CiscoCodecStatus.RoomPreset> tempPresets = tempCodecStatus.Status.RoomPreset;
                    ...
                    if (tempPresets.Count > 0)
                    {
                        ...
                        foreach (CiscoCodecStatus.RoomPreset camPreset in tempPresets)
                        {
                            ...
                            if (existingPreset != null)
                            {
                                ...
                                foreach (JToken jPreset in roomPresets)
                        ...
                        // Generecise the list
                        NearEndPresets = existingRoomPresets
                    ...
                    if (!_syncState.InitialStatusMessageWasReceived)
                    {
                        ...
                        if (!_syncState.InitialConfigurationMessageWasReceived) SendText("xConfiguration");
                    }
                }
                else if (response.IndexOf("\"Configuration\":{") > -1 || response.IndexOf("\"Configuration\": {") > -1)
                {
                    ...
                    if (!_syncState.InitialConfigurationMessageWasReceived)
                    {
                        ...
                        if (!_syncState.FeedbackWasRegistered) SendText(_cliFeedbackRegistrationExpression);
                    }
                }
                else if (response.IndexOf("\"Event\":{") > -1 || response.IndexOf("\"Event\": {") > -1)
                {
                    ...
                    else if (response.IndexOf("\"Bookings\":{") > -1 ||
                             response.IndexOf("\"Bookings\": {") > -1) // The list has changed, reload it
                    {
                        GetBookings(null);
                    }
                             ...
                             response.IndexOf("\"UserInterface\": {") > -1) // External Source Trigger
                    {
                        ...
                        Debug.Console(2, this, "*** Got an External Source Selection {0} {1}", eventReceived,
                            ...
                            eventReceived.Event.UserInterface.Presentation.ExternalSource.Selected.SourceIdentifier
                                .Value);
                         ...
                         response.IndexOf("\"CommandResponse\": {") > -1)
                {
                    ...
                    if (response.IndexOf("\"PeripheralsHeartBeatResult\":{") > -1 ||
                        response.IndexOf("\"PeripheralsHeartBeatResult\": {") > -1)
                    {
                        if (!_syncState.JsonResponseModeSet)
                            _syncState.JsonResponseModeMessageReceived();
                        if (!_syncState.InitialStatusMessageWasReceived)
                    ...
                    else if (response.IndexOf("\"CallHistoryRecentsResult\":{") > -1 ||
                             response.IndexOf("\"CallHistoryRecentsResult\": {") > -1)
                    {
                        CiscoCallHistory.RootObject codecCallHistory = new CiscoCallHistory.RootObject();
                    ...
                    {
                        CiscoCodecPhonebook.RootObject codecPhonebookResponse = new CiscoCodecPhonebook.RootObject();
                        ...
                        if (_getZoomContact)
                        {
                            ...
                            if (codecPhonebookResponse.CommandResponse.PhonebookSearchResult.Contact != null)
                                foreach (CiscoCodecPhonebook.Contact contact in codecPhonebookResponse.CommandResponse
                                             .PhonebookSearchResult.Contact)
                        ...
                        else if (!PhonebookSyncState.InitialPhonebookFoldersWasReceived)
                        {
                            ...
                            PhonebookSyncState.SetPhonebookHasFolders(codecPhonebookResponse.CommandResponse
                                .PhonebookSearchResult.Folder.Count > 0);

                            if (PhonebookSyncState.PhonebookHasFolders)
                                DirectoryRoot.AddFoldersToDirectory(
                                    CiscoCodecPhonebook.GetRootFoldersFromSearchResult(codecPhonebookResponse
                                        .CommandResponse.PhonebookSearchResult));
                        ...
                        else if (!PhonebookSyncState.NumberOfContactsWasReceived)
                        {
                            ...
                            PhonebookSyncState.SetNumberOfContacts(int.Parse(codecPhonebookResponse.CommandResponse
                                .PhonebookSearchResult.ResultInfo.TotalRows.Value));

                            DirectoryRoot.AddContactsToDirectory(
                                CiscoCodecPhonebook.GetRootContactsFromSearchResult(codecPhonebookResponse
                                    .CommandResponse.PhonebookSearchResult));
                        ...
                        else if (PhonebookSyncState.InitialSyncComplete)
                        {
                            ...
                            if (codecPhonebookResponse.CommandResponse.PhonebookSearchResult.ResultInfo.TotalRows
                                    .Value != "0")
                                directoryResults =
                                    CiscoCodecPhonebook.ConvertCiscoPhonebookToGeneric(codecPhonebookResponse
                                        .CommandResponse.PhonebookSearchResult);
                    ...
                    {
                        CiscoCodecBookings.RootObject codecBookings = new CiscoCodecBookings.RootObject();
                        ...
                        if (codecBookings.CommandResponse.BookingsListResult.ResultInfo.TotalRows.Value != "0")
                            CodecSchedule.Meetings =
                                CiscoCodecBookings.GetGenericMeetingsFromBookingResult(codecBookings.CommandResponse
                                    .BookingsListResult.Booking);
            ...
            catch (Exception ex)
            {
                ...
                if (ex is JsonReaderException) Debug.Console(1, this, "Received malformed response from codec.");
                //Communication.Disconnect();
                //Initialize();
            }
        }
        ...
        private void EvalutateDisconnectEvent(CiscoCodecEvents.RootObject eventReceived)
        {
            ...
            {
                CodecActiveCallItem tempActiveCall =
        ...
        public string GetCallId()
        {
            ...
            {
                int lastCallIndex = ActiveCalls.Count - 1;
        ...
        public void CallZoomMeeting()
        {
            ...
            if (ZoomContact != null)
            {
                ...
                buildString[0] = _zoomMeeting != null && _zoomMeeting.Length > 1
                    ? _zoomMeeting.PadLeft(11, '0')
                    : "0";
                buildString[1] = _zoomPassword != null && _zoomPassword.Length > 1 ? _zoomPassword : "";
                string dialString = "";
                for (int i = 0; i < buildString.Length; i++)
                {
                    if (i > 0) dialString = dialString + ".";
        ...
        {
            // Get Phonebook Folders (determine local/corporate from config, and set results limit)
            EnqueueCommand(string.Format("xCommand Phonebook Search PhonebookType: {0} ContactType: Contact Limit: {1}",
                _phonebookMode, _phonebookResultsLimit));
        }
        ...
        {
            EnqueueCommand(string.Format(
                "xCommand Phonebook Search FolderId: {0} PhonebookType: {1} ContactType: Any Limit: {2}", folderId,
                _phonebookMode, _phonebookResultsLimit));
        }
        ...
        {
            CodecDirectory currentDirectory = new CodecDirectory();

            if (DirectoryBrowseHistory.Count > 0)
            {
                int lastItemIndex = DirectoryBrowseHistory.Count - 1;
                CodecDirectory parentDirectoryContents = DirectoryBrowseHistory[lastItemIndex];
        ...
        private void PrintDirectory(CodecDirectory directory)
        {
            if (Debug.Level > 0)
            {
                ...
                foreach (DirectoryItem item in directory.CurrentDirectoryResults)
                    ...
                    else if (item is DirectoryContact) Debug.Console(1, this, "{0}", item.Name);
        ...
        {
            foreach (Call c in meeting.Calls) Dial(c.Number, c.Protocol, c.CallRate, c.CallType, meeting.Id);
        }
        ...
        {
            EnqueueCommand(string.Format(
                "xCommand Dial Number: \"{0}\" Protocol: {1} CallRate: {2} CallType: {3} BookingId: {4}", number,
                protocol, callRate, callType, meetingId));
        }
        ...
        public void JoinAllCalls()
        {
            ...
            if (ids.Length > 0) EnqueueCommand(string.Format("xCommand Call Join {0}", ids.ToString()));
        }
        ...
        public override void StartSharing()
        {
            ...
            if (_desiredPresentationSource > 0)
                EnqueueCommand(string.Format("xCommand Presentation Start PresentationSource: {0} SendingMode: {1}",
                    _desiredPresentationSource, sendingMode));
        }
        ...
        {
            CiscoCodecJoinMap joinMap = new CiscoCodecJoinMap(joinStart);
        ...
        public void LinkCiscoCodecToApi(BasicTriList trilist, CiscoCodecJoinMap joinMap)
        {
            ...
            IHasDoNotDisturbMode dndCodec = this as IHasDoNotDisturbMode;
            if (dndCodec != null)
            {
            ...
            IHasHalfWakeMode halfwakeCodec = this as IHasHalfWakeMode;
        ...
        public void SelfviewPipPositionToggle()
        {
            if (_currentSelfviewPipPosition != null)
            {
                ...
                if (nextPipPositionIndex >=
                    SelfviewPipPositions.Count) // Check if we need to loop back to the first item in the list
        ...
        {
            EnqueueCommand(string.Format("xCommand Video Layout LayoutFamily Set Target: local LayoutFamily: {0}",
                layout.Command));
        }
        ...
        public void LocalLayoutToggle()
        {
            ...
            {
                int nextLocalLayoutIndex = LocalLayouts.IndexOf(_currentLocalLayout) + 1;

                if (nextLocalLayoutIndex >=
                    LocalLayouts.Count) // Check if we need to loop back to the first item in the list
        ...
        {
            if (CameraIsOffFeedback.BoolValue) CameraMuteOff();
        ...
        private void SetUpCameras(List<CameraInfo> cameraInfo)
        {
            ...
            int camCount = CodecStatus.Status.Cameras.Camera.Count;

            // Deal with the case of 1 or no reported cameras
            if (camCount <= 1)
            {
                CiscoSparkCamera internalCamera = new CiscoSparkCamera(Key + "-camera1", "Near End", this, 1);
            ...
            else
            {
                ...
                {
                    CiscoCodecStatus.Camera cam = CodecStatus.Status.Cameras.Camera[i];

                    uint id = (uint)i;
                    string name = string.Format("Camera {0}", id + 1);

                    // Check for a config object that matches the camera number
                    CameraInfo camInfo = cameraInfo.FirstOrDefault(c => c.CameraNumber == i + 1);
                    if (camInfo != null) id = (uint)camInfo.SourceId;

                    if (_config.CameraNames != null)
                    {
                        string nameKey = (i + 1).ToString();
                        if (_config.CameraNames.ContainsKey(nameKey)) name = _config.CameraNames[nameKey];
                    }

                    string key = string.Format("{0}-camera{1}", Key, id);
                    CiscoSparkCamera camera = new CiscoSparkCamera(key, name, this, id);

                    if (cam.Capabilities != null) camera.SetCapabilites(cam.Capabilities.Options.Value);
            ...
            CiscoFarEndCamera farEndCamera = new CiscoFarEndCamera(Key + "-cameraFar", "Far End", this);
            ...
            {
                string label = string.Format("Far End Preset {0}", i);
        ...
        public void SelectCamera(string key)
        {
            ...
            CiscoSparkCamera ciscoCam = camera as CiscoSparkCamera;
            if (ciscoCam != null)
                EnqueueCommand(
                    string.Format("xCommand Video Input SetMainVideoSource SourceId: {0}", ciscoCam.CameraId));
        }
        ...
        public class CiscoCodecInfo : VideoCodecInfo
        {
            ...
            public override bool MultiSiteOptionIsEnabled
            {
                get
                {
                    if (!string.IsNullOrEmpty(CodecStatus.Status.SystemUnit.Software.OptionKeys.MultiSite.Value) &&
                        CodecStatus.Status.SystemUnit.Software.OptionKeys.MultiSite.Value.ToLower() == "true")
                        return true;
                    else
                        return false;
                }
            ...
            public override string E164Alias
            {
                get
                {
                    if (CodecConfiguration.Configuration.H323 != null &&
                        CodecConfiguration.Configuration.H323.H323Alias.E164 != null)
            ...
            public override string H323Id
            {
                get
                {
                    if (CodecConfiguration.Configuration.H323 != null && CodecConfiguration.Configuration.H323
                                                                          .H323Alias != null
            ...
            public override string SipPhoneNumber
            {
                get
                {
                    ...
                    {
                        Match match = Regex.Match(CodecStatus.Status.SIP.Registration[0].URI.Value,
                            @"(\d+)"); // extract numbers only
                        if (match.Success)
                        {
                            Debug.Console(1, "Extracted phone number as '{0}' from string '{1}'", match.Groups[1].Value,
                                CodecStatus.Status.SIP.Registration[0].URI.Value);
                            return match.Groups[1].Value;
                        }
                        else
                        {
                            Debug.Console(1, "Unable to extract phone number from string: '{0}'",
                                CodecStatus.Status.SIP.Registration[0].URI.Value);
                            return string.Empty;
                        }
                    }
        ...
        {
            EnqueueCommand(string.Format("xCommand Call FarEndControl RoomPreset Activate CallId: {0} PresetId: {1}",
                GetCallId(), preset));
        }
        ...
        {
            int id = 2;
            if (connectorId.ToLower() == "hdmiin3") id = 3;
    ...
    public class CodecSyncState : IKeyed
    {
        ...
        private void ProcessQueuedCommands()
        {
            ...
            {
                string query = _commandQueue.Dequeue();
    ...
    public class CiscoSparkCodecFactory : EssentialsDeviceFactory<CiscoSparkCodec>
    {
        public CiscoSparkCodecFactory()
        {
            TypeNames = new List<string>
                { "ciscospark", "ciscowebex", "ciscowebexpro", "ciscoroomkit", "ciscosparkpluscodec" };
        }
