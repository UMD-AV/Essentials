...
namespace NvxEpi.Application.Entities
{
    public class NvxApplicationVideoReceiver : EssentialsDevice, IOnline
    {
            ...
            IEnumerable<NvxApplicationVideoTransmitter> transmitters) : base(key)
        {
            ...
            AddPostActivationAction(() =>
            {
                RoutingOutputPort port = Device.OutputPorts[SwitcherForHdmiOutput.Key];
                if (port == null)
            ...
            AddPostActivationAction(() =>
            {
                ...
                VideoName = new StringFeedback(() =>
                    string.IsNullOrEmpty(config.VideoName) ? Device.Name : config.VideoName);
            ...
            AddPostActivationAction(() =>
            {
                StringFeedback feedback = Device.Feedbacks[CurrentVideoStream.RouteNameKey] as StringFeedback;
                if (feedback == null)
                    throw new NullReferenceException(CurrentVideoStream.RouteNameKey);

                IntFeedback currentRouteFb = new IntFeedback(() =>
                {
            ...
            AddPostActivationAction(() =>
            {
                StringFeedback currentRouteNameFb = new StringFeedback(() =>
                {
