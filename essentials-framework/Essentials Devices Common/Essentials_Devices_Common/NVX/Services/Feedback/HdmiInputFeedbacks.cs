        ...
        public static StringFeedback GetFeedback(DmNvxBaseClass device, uint _inputNumber)
        {
            ...
            StringFeedback feedback = new StringFeedback(string.Format(Key, _inputNumber), () =>
            {
                string resolution = string.Format("{0}x{1}@{2}",
                    device.HdmiIn[_inputNumber].VideoAttributes.HorizontalResolutionFeedback.UShortValue,
                    device.HdmiIn[_inputNumber].VideoAttributes.VerticalResolutionFeedback.UShortValue,
                    device.HdmiIn[_inputNumber].VideoAttributes.FramesPerSecondFeedback.UShortValue);
                return resolution;
            });
