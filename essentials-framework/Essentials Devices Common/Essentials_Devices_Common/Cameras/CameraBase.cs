...
namespace PepperDash.Essentials.Devices.Common.Cameras
{
    ...
    public abstract class CameraBase : ReconfigurableDevice, IRoutingOutputs
    {
        ...
        public bool CanPan
        {
            get { return (Capabilities & eCameraCapabilities.Pan) == eCameraCapabilities.Pan; }
        }

        [JsonProperty("canTilt", NullValueHandling = NullValueHandling.Ignore)]
        public bool CanTilt
        {
            get { return (Capabilities & eCameraCapabilities.Tilt) == eCameraCapabilities.Tilt; }
        }

        [JsonProperty("canZoom", NullValueHandling = NullValueHandling.Ignore)]
        public bool CanZoom
        {
            get { return (Capabilities & eCameraCapabilities.Zoom) == eCameraCapabilities.Zoom; }
        }

        [JsonProperty("canFocus", NullValueHandling = NullValueHandling.Ignore)]
        public bool CanFocus
        {
            get { return (Capabilities & eCameraCapabilities.Focus) == eCameraCapabilities.Focus; }
        }
            ...
            EiscApiAdvanced bridge)
        {
            ...
            ICommunicationMonitor commMonitor = cameraDevice as ICommunicationMonitor;
            commMonitor.CommunicationMonitor.IsOnlineFeedback.LinkInputSig(
                trilist.BooleanInput[joinMap.IsOnline.JoinNumber]);

            IHasCameraPtzControl ptzCamera = cameraDevice as IHasCameraPtzControl;
            ...
            IHasPowerControl powerCamera = cameraDevice as IHasPowerControl;
            if (powerCamera != null)
            {
                ...
                IHasPowerControlWithFeedback powerFbCamera = powerCamera as IHasPowerControlWithFeedback;
                if (powerFbCamera != null)
                {
                    powerFbCamera.PowerIsOnFeedback.LinkInputSig(trilist.BooleanInput[joinMap.PowerOn.JoinNumber]);
                    powerFbCamera.PowerIsOnFeedback.LinkComplementInputSig(
            ...
            {
                ICommunicationMonitor monitoredCamera = cameraDevice as ICommunicationMonitor;
            ...
            if (cameraDevice is IHasCameraPresets)
            {
                ...
                for (int i = 0; i < joinMap.NumberOfPresets.JoinSpan; i++)
                {
                    ...
                    {
                        string label = trilist.GetString((ushort)(joinMap.PresetLabelStart.JoinNumber + tempNum));
        ...
        private void SendCameraPresetNamesToApi(IHasCameraPresets presetsCamera, CameraControllerJoinMap joinMap,
            BasicTriList trilist)
        {
            for (int i = 1; i <= joinMap.NumberOfPresets.JoinNumber; i++)
            {
                ...
                CameraPreset preset = presetsCamera.Presets.FirstOrDefault(p => p.ID.Equals(i));
    ...
    public class CameraPropertiesConfig
    {
        ...
        [JsonProperty("supportsAutoMode")] public bool SupportsAutoMode { get; set; }

        [JsonProperty("supportsOffMode")] public bool SupportsOffMode { get; set; }

        [JsonProperty("presets")] public List<CameraPreset> Presets { get; set; }
    }
}