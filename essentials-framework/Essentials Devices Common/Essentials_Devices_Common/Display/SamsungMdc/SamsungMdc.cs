...
namespace PepperDash.Essentials.Devices.Displays
{
        ...
        IBridgeAdvanced, IDeviceInfoProvider
    {
        ...
        {
            SamsungDisplayControllerJoinMap joinMap = new SamsungDisplayControllerJoinMap(joinStart);
            ...
            else
            {
                foreach (FriendlyName name in _config.FriendlyNames)
                {
                    try
                    {
                        ...
                        trilist.StringInput[(ushort)(joinMap.InputNamesOffset.JoinNumber + count)].StringValue =
                            name.Name;

                        int index = InputPorts.FindIndex(n => n.Key == name.InputKey);
                        InputFeedback[index]
                            .LinkInputSig(trilist.BooleanInput[joinMap.InputSelectOffset.JoinNumber + (uint)count]);
                        count++;
                    }
                    catch
                    {
        ...
        {
            long pollInterval = _pollIntervalMs > 0 ? _pollIntervalMs : 30000;
        ...
        private void Communication_BytesReceived(object sender, GenericCommMethodReceiveBytesArgs e)
        {
            ...
            catch (Exception ex)
            {
                Debug.Console(0, this,
                    string.Format("Samsung MDC exception parsing feedback: {0}, {1}", ex.Message,
        ...
        private void ParseMessage(byte[] message)
        {
            ...
            try
            {
                ...
                switch (command)
                {
                    ...
                    case 0x1B: //network info
                    {
                        ...
                        if (message[4] == 0x82)
                        {
                            byte[] ipAddressInfo = new byte[length - 1];
                        ...
                        if (message[4] == 0x81)
                        {
                            byte[] macInfo = new byte[length - 1];
            ...
            {
                Debug.Console(0, this, "Exception parsing message: {0}, {1}", ComTextHelper.GetEscapedText(message),
                    ex.Message);
            }
        }
        ...
        {
            string version = Encoding.UTF8.GetString(firmware, 0, 12);
        ...
        {
            string serialNumberString = Encoding.UTF8.GetString(serialNumber, 0, serialNumber.Length);
        ...
        {
            RoutingInputPort newInput = InputPorts.FirstOrDefault(i => i.FeedbackMatchObject.Equals(b));
            if (newInput != null)
            {
                _currentInputPort = newInput;
                CurrentInputFeedback.FireUpdate();
                string key = newInput.Key;
                switch (key)
                {
                    case "hdmiIn1":
                        _inputNumber = 1;
