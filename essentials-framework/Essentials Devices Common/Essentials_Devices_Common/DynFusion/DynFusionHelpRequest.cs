    ...
    public class DynFusionHelpRequest
    {
        ...
        public void CancelRequest(string id)
        {
            if (!string.IsNullOrEmpty(id))
            {
                ...
                try
                {
                    foreach (string req in helpRequestIds)
                    {
                        if (req.ToString().StartsWith(id))
                        {
                            helpSig.InputSig.StringValue = "<HelpRequest><ID>" + req.ToString() +
                                                           "</ID><Message>cancel</Message><Type>cancel</Type></HelpRequest>";
                        }
                    }
        ...
        public void CancelAll()
        {
            ...
            try
            {
                if (helpRequestIds != null)
                {
                    ...
                    {
                        helpSig.InputSig.StringValue = "<HelpRequest><ID>" + uniqueId +
                                                       "</ID><Message>cancel</Message><Type>cancel</Type></HelpRequest>";
                    }
                }
        ...
        public void ParseFeedback(string data)
        {
            ...
            try
            {
                ...
                XmlDoc.LoadXml(data);
                XmlNode xmlId = XmlDoc.SelectSingleNode("HelpRequest/ID");
                if (xmlId != null)
                {
                    string message = "";
                    ushort active = 1;
                    ...
                    XmlNode xmlMessage = XmlDoc.SelectSingleNode("HelpRequest/Message");
                    if (xmlMessage != null)
                    {
                    ...
                    XmlNode xmlType = XmlDoc.SelectSingleNode("HelpRequest/Type");
                    if (xmlType != null)
                    {
                ...
                {
                    XmlNodeList openIds = XmlDoc.SelectNodes("HelpRequest/OpenItems/ID");
                    if (openIds.Count > 0)
                    {
