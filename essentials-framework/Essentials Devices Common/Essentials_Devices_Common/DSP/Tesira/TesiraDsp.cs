...
namespace Tesira_DSP_EPI
{
    public class TesiraDsp : EssentialsBridgeableDevice, IHasDspPresets, ICommunicationMonitor
    {
        ...
        public bool IsSubscribed
        {
            get
            {
                bool subscribeTracker = ControlPointList.All(subscribedComponent => subscribedComponent.IsSubscribed);
            ...
            : base(key, name)
        {
            ...
            CrestronConsole.AddNewConsoleCommand(CommandQueue.EnqueueCommand, "send" + Key, "",
        ...
        private void CreatePresets(TesiraDspPropertiesConfig props)
        {
            ...
            {
                TesiraDspPresets value = preset.Value;
                TesiraPreset tesiraPreset = new TesiraPreset(preset.Value);
                Presets.Add(tesiraPreset);
                TesiraPresets.Add(tesiraPreset);
                Debug.Console(2, this, "Added Preset {0} {1}", value.Label, value.PresetName);
            }

            TesiraDspPresetDevice presetDevice = new TesiraDspPresetDevice(this);
        ...
        private void CreateFaders(TesiraDspPropertiesConfig props)
        {
            ...
            foreach (KeyValuePair<string, TesiraFaderControlBlockConfig> block in props.FaderControlBlocks)
            {
                string key = block.Key;
                Debug.Console(2, this, "faderControlBlock Key - {0}", key);
                TesiraFaderControlBlockConfig value = block.Value;

                Faders.Add(key, new TesiraDspFaderControl(key, value, this));
                Debug.Console(2, this, "Added faderControlPoint {0} levelTag: {1} muteTag: {2}", key,
        ...
        private void CreateCrosspoints(TesiraDspPropertiesConfig props)
        {
            ...
            foreach (KeyValuePair<string, TesiraCrosspointStateBlockConfig> mixer in props.CrosspointStateControlBlocks)
            {
                string key = mixer.Key;
                TesiraCrosspointStateBlockConfig value = mixer.Value;
        ...
        private void CreateMeters(TesiraDspPropertiesConfig props)
        {
            ...
            {
                string key = meter.Key;
                TesiraMeterBlockConfig value = meter.Value;
        ...
        private void CreateStates(TesiraDspPropertiesConfig props)
        {
            ...
            {
                string key = block.Key;
                TesiraStateControlBlockConfig value = block.Value;
        ...
        private void CreateDialers(TesiraDspPropertiesConfig props)
        {
            ...
            foreach (KeyValuePair<string, TesiraDialerControlBlockConfig> block in props.DialerControlBlocks)
            {
                ...
                TesiraDialerControlBlockConfig value = block.Value;
        ...
        private void CreateSwitchers(TesiraDspPropertiesConfig props)
        {
            ...
            foreach (KeyValuePair<string, TesiraSwitcherControlBlockConfig> block in props.SwitcherControlBlocks)
            {
                ...
                TesiraSwitcherControlBlockConfig value = block.Value;
        ...
        private void CreateSourceSelectors(TesiraDspPropertiesConfig props)
        {
                         ...
                         .SourceSelectorControlBlocks)
            {
                ...
                TesiraSourceSelectorControlBlockConfig value = block.Value;

                SourceSelectors.Add(key, new TesiraDspSourceSelector(key, value, this));
                Debug.Console(2, this, "Added TesiraSwitcher {0} InstanceTag {1}", key,
                    value.SourceSelectorInstanceTag);
        ...
        private void CreateRouters(TesiraDspPropertiesConfig props)
        {
            ...
            foreach (KeyValuePair<string, TesiraRouterControlBlockConfig> block in props.RouterControlBlocks)
            {
                string key = block.Key;
                Debug.Console(2, this, "RouterControlBlock Key - {0}", key);
                TesiraRouterControlBlockConfig value = block.Value;
        ...
        private void CheckWatchDog()
        {
            try
            {
                ...
                if (!WatchDogSniffer)
                {
                    ...
                    ISubscribedComponent watchDogSubject = ControlPointList[random.Next(0, ControlPointList.Count)];
                    if (!watchDogSubject.Enabled)
                    {
                        Debug.Console(1, this,
                            "The Watchdog was wrong - that's just an old shoe.  Nothing is subscribed.");
                        return;
                    }
        ...
        private void Port_LineReceived(object dev, GenericCommMethodReceiveTextArgs args)
        {
            ...
            try
            {
                ...
                else if (args.Text.IndexOf("! ", StringComparison.Ordinal) >= 0)
                {
                    ...
                    foreach (ISubscribedComponent component in from component in ControlPointList
                             let item = component
                             from n in item.CustomNames.Where(n => n == customName)
                             select component)
                    {
                        if (component == null)
                        {
                ...
                else if (args.Text.IndexOf("+OK", StringComparison.Ordinal) == 0)
                {
                    ...
                    if (args.Text ==
                        "+OK") // Check for a simple "+OK" only 'ack' repsonse or a list response and ignore
        ...
        public void RunPresetNumber(ushort n)
        {
            ...
            foreach (TesiraPreset preset in Presets.OfType<TesiraPreset>().Where(preset => preset.Index == n))
            {
        ...
        public void RecallPreset(IDspPreset preset)
        {
            ...
            TesiraPreset tesiraPreset = preset as TesiraPreset;
            ...
            else
            {
                ...
                {
                    Debug.Console(2, this, "Preset {0} has an invalid presetId {1}", tesiraPreset.Name,
                        tesiraPreset.PresetData.PresetId);
                    return;
                }
        ...
        {
            ISubscribedComponent controlPoint = ControlPointList[index];
            if (controlPoint != null) UnsubscribeFromComponent(controlPoint);
            int newIndex = index + 1;
            Debug.Console(2, this, "NewIndex == {0} and ControlPointListCount == {1}", newIndex,
        ...
        {
            IVolumeComponent data = faders[index];
            if (data != null) data.GetMaxLevel();
            int indexerOutput = index + 1;
        ...
        {
            Debug.Console(2, this, "LocalQueue Size = {0} and Command Queue {1} in Progress",
                CommandQueue.LocalQueue.Count, CommandQueue.CommandQueueInProgress ? "is" : "is not");
        ...
        private void SubscribeToComponentByIndex(int indexer)
        {
            ...
            ISubscribedComponent data = ControlPointList[indexer];
            SubscribeToComponent(data);
            int indexerOutput = indexer + 1;
            Debug.Console(2, this, "Indexer = {0} : Count = {1} : ControlPointList", indexerOutput,
                ControlPointList.Count());
        ...
        {
            TesiraDspDeviceJoinMapAdvanced deviceJoinMap = new TesiraDspDeviceJoinMapAdvanced(joinStart);
            trilist.StringInput[deviceJoinMap.Name.JoinNumber].StringValue = Name;

            TesiraDialerJoinMapAdvanced dialerJoinMap = new TesiraDialerJoinMapAdvanced(joinStart);
            TesiraFaderJoinMapAdvanced faderJoinMap = new TesiraFaderJoinMapAdvanced(joinStart);
            TesiraStateJoinMapAdvanced stateJoinMap = new TesiraStateJoinMapAdvanced(joinStart);
            ...
            foreach (KeyValuePair<string, TesiraDspFaderControl> item in Faders)
            {
                TesiraDspFaderControl channel = item.Value;
                uint? data = channel.BridgeIndex;
                if (data == null) continue;
                uint x = (uint)data;
                //var TesiraChannel = channel.Value as Tesira.DSP.EPI.TesiraDspLevelControl;
                Debug.Console(2, "TesiraChannel {0} connect", x);

                IBasicVolumeWithFeedback genericChannel = channel as IBasicVolumeWithFeedback;
                ...
                genericChannel.MuteFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[faderJoinMap.MuteOff.JoinNumber + x]);
                genericChannel.VolumeLevelFeedback.LinkInputSig(
                    trilist.UShortInput[faderJoinMap.Volume.JoinNumber + x]);
                ...
                trilist.SetSigFalseAction(faderJoinMap.EnableLevelSend.JoinNumber + x, () =>
                {
                    CrestronEnvironment.Sleep(500);
                    genericChannel.SetVolume(trilist.UShortOutput[faderJoinMap.Volume.JoinNumber + x].UShortValue);
                });
            ...
            foreach (KeyValuePair<string, TesiraDspStateControl> item in States)
            {
                TesiraDspStateControl state = item.Value;
                uint? data = state.BridgeIndex;
                if (data == null) continue;
            ...
            foreach (KeyValuePair<string, TesiraDspSwitcher> item in Switchers)
            {
                TesiraDspSwitcher switcher = item.Value;
                ...
                IRoutingWithFeedback s = switcher as IRoutingWithFeedback;
            ...
            //Source Selectors
            Debug.Console(2, this, "There are {0} SourceSelector Control Points", Switchers.Count());
            foreach (KeyValuePair<string, TesiraDspRouter> item in Routers)
            {
                TesiraDspRouter switcher = item.Value;
                uint? data = switcher.BridgeIndex;
                if (data == null) continue;
                uint y = (uint)data;
                ushort x = (ushort)(((y - 1) * 2) + 1);
            ...
            //Source Selectors
            Debug.Console(2, this, "There are {0} SourceSelector Control Points", Switchers.Count());
            foreach (KeyValuePair<string, TesiraDspSourceSelector> item in SourceSelectors)
            {
                TesiraDspSourceSelector switcher = item.Value;
                uint? data = switcher.BridgeIndex;
                if (data == null) continue;
                uint y = (uint)data;
                ushort x = (ushort)(((y - 1) * 2) + 1);
            ...
            foreach (IDspPreset preset in Presets)
            {
                TesiraPreset p = preset as TesiraPreset;
                if (p == null) continue;
                int runPresetIndex = p.PresetData.PresetIndex;
                int presetIndex = runPresetIndex;
                trilist.StringInput[(uint)(presetJoinMap.PresetNameFeedback.JoinNumber + presetIndex)].StringValue =
            ...
            uint lineOffset = 0;
            foreach (KeyValuePair<string, TesiraDspDialer> line in Dialers)
            {
                TesiraDspDialer dialer = line.Value;
                uint? bridgeIndex = dialer.BridgeIndex;
                if (bridgeIndex == null)
                {
                ...
                uint dialerLineOffset = lineOffset;
                ...
                trilist.SetSigTrueAction(dialerJoinMap.DoNotDisturbToggle.JoinNumber + dialerLineOffset,
                    dialer.DoNotDisturbToggle);
                trilist.SetSigTrueAction(dialerJoinMap.DoNotDisturbOn.JoinNumber + dialerLineOffset,
                    dialer.DoNotDisturbOn);
                trilist.SetSigTrueAction(dialerJoinMap.DoNotDisturbOff.JoinNumber + dialerLineOffset,
                    dialer.DoNotDisturbOff);
                trilist.SetSigTrueAction(dialerJoinMap.AutoAnswerToggle.JoinNumber + dialerLineOffset,
                    dialer.AutoAnswerToggle);
                trilist.SetSigTrueAction(dialerJoinMap.AutoAnswerOn.JoinNumber + dialerLineOffset, dialer.AutoAnswerOn);
                trilist.SetSigTrueAction(dialerJoinMap.AutoAnswerOff.JoinNumber + dialerLineOffset,
                    dialer.AutoAnswerOff);
                ...
                trilist.SetStringSigAction(dialerJoinMap.DialString.JoinNumber + dialerLineOffset,
                    dialer.SetDialString);

                dialer.DoNotDisturbFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.DoNotDisturbToggle.JoinNumber + dialerLineOffset]);
                dialer.DoNotDisturbFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.DoNotDisturbOn.JoinNumber + dialerLineOffset]);
                dialer.DoNotDisturbFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[dialerJoinMap.DoNotDisturbOff.JoinNumber + dialerLineOffset]);

                dialer.OffHookFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.KeyPadDial.JoinNumber + dialerLineOffset]);
                dialer.OffHookFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.OffHook.JoinNumber + dialerLineOffset]);
                dialer.OffHookFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[dialerJoinMap.OnHook.JoinNumber + dialerLineOffset]);
                dialer.IncomingCallFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.IncomingCall.JoinNumber + dialerLineOffset]);

                dialer.AutoAnswerFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.AutoAnswerToggle.JoinNumber + dialerLineOffset]);
                dialer.AutoAnswerFeedback.LinkInputSig(
                    trilist.BooleanInput[dialerJoinMap.AutoAnswerOn.JoinNumber + dialerLineOffset]);
                dialer.AutoAnswerFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[dialerJoinMap.AutoAnswerOff.JoinNumber + dialerLineOffset]);

                dialer.NameFeedback.LinkInputSig(
                    trilist.StringInput[dialerJoinMap.Label.JoinNumber + dialerLineOffset]);
                dialer.DisplayNumberFeedback.LinkInputSig(
                    trilist.StringInput[dialerJoinMap.DisplayNumber.JoinNumber + dialerLineOffset]);

                dialer.DialStringFeedback.LinkInputSig(
                    trilist.StringInput[dialerJoinMap.DialString.JoinNumber + dialerLineOffset]);
                dialer.CallerIdNumberFeedback.LinkInputSig(
                    trilist.StringInput[dialerJoinMap.CallerIdNumberFb.JoinNumber + dialerLineOffset]);
                dialer.CallerIdNameFeedback.LinkInputSig(
                    trilist.StringInput[dialerJoinMap.CallerIdNameFb.JoinNumber + dialerLineOffset]);
                dialer.LastDialedFeedback.LinkInputSig(
                    trilist.StringInput[dialerJoinMap.LastNumberDialerFb.JoinNumber + dialerLineOffset]);

                dialer.HoldCallFeedback.LinkInputSig(trilist.BooleanInput[dialerJoinMap.HoldCall.JoinNumber]);
                dialer.HoldCallFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[dialerJoinMap.ResumeCall.JoinNumber]);
                dialer.HoldCallFeedback.LinkInputSig(trilist.BooleanInput[dialerJoinMap.HoldToggle.JoinNumber]);


                dialer.CallStateFeedback.LinkInputSig(
                    trilist.UShortInput[dialerJoinMap.CallState.JoinNumber + dialerLineOffset]);

                lineOffset += 50;
            }

            Debug.Console(2, this, "There are {0} Meter Control Points", Meters.Count);
            foreach (KeyValuePair<string, TesiraDspMeter> item in Meters)
            {
                TesiraDspMeter meter = item.Value;
                uint? data = meter.BridgeIndex;
                if (data == null) continue;
                uint x = (uint)(data - 1);
            ...
            foreach (KeyValuePair<string, TesiraDspCrosspointState> item in CrosspointStates)
            {
                ...
                TesiraDspCrosspointState channel = item.Value;
                uint? data = channel.BridgeIndex;
                if (data == null) continue;
                ...
                xpointState.CrosspointStateFeedback.LinkInputSig(
                    trilist.BooleanInput[(uint)(crosspointStateJoinMap.On.JoinNumber + joinOffset)]);

                trilist.SetSigTrueAction((uint)(crosspointStateJoinMap.Toggle.JoinNumber + joinOffset),
                    xpointState.StateToggle);
                trilist.SetSigTrueAction((uint)(crosspointStateJoinMap.On.JoinNumber + joinOffset),
                    xpointState.StateOn);
                trilist.SetSigTrueAction((uint)(crosspointStateJoinMap.Off.JoinNumber + joinOffset),
                    xpointState.StateOff);
            }

            Debug.Console(2, this, "There are {0} Room Combiner Control Points", RoomCombiners.Count);
            //x = 0;
            foreach (KeyValuePair<string, TesiraDspRoomCombiner> item in RoomCombiners)
            {
                TesiraDspRoomCombiner roomCombiner = item.Value;
                uint? data = roomCombiner.BridgeIndex;
                if (data == null) continue;
                uint y = (uint)data;
                ...
                IBasicVolumeWithFeedback genericChannel = roomCombiner as IBasicVolumeWithFeedback;
                ...
                roomCombiner.VisibleFeedback.LinkInputSig(
                    trilist.BooleanInput[roomCombinerJoinMap.Visible.JoinNumber + x]);
                roomCombiner.TypeFeedback.LinkInputSig(trilist.UShortInput[roomCombinerJoinMap.Type.JoinNumber + x]);
                roomCombiner.PermissionsFeedback.LinkInputSig(
                    trilist.UShortInput[roomCombinerJoinMap.Permissions.JoinNumber + x]);
                roomCombiner.RoomGroupFeedback.LinkInputSig(
                    trilist.UShortInput[roomCombinerJoinMap.Group.JoinNumber + x]);

                genericChannel.MuteFeedback.LinkInputSig(
                    trilist.BooleanInput[roomCombinerJoinMap.MuteToggle.JoinNumber + x]);
                genericChannel.MuteFeedback.LinkInputSig(
                    trilist.BooleanInput[roomCombinerJoinMap.MuteOn.JoinNumber + x]);
                genericChannel.MuteFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[roomCombinerJoinMap.MuteOff.JoinNumber + x]);
                genericChannel.VolumeLevelFeedback.LinkInputSig(
                    trilist.UShortInput[roomCombinerJoinMap.Volume.JoinNumber + x]);
