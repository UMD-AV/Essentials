...
namespace Tesira_DSP_EPI
{
    public class TesiraQueue
    {
        ...
        {
            Debug.Console(2, Parent, "[EqueueCommand(String)] - Command Queue {0} in progress.",
                CommandQueueInProgress ? "is" : "is not");

            LocalQueue.Enqueue(command);
            Debug.Console(2, Parent,
                "[EqueueCommand(String)] - Command Enqueued '{0}'.  CommandQueue has '{1}' Elements.", command,
                LocalQueue.Count);
        ...
        public void SendNextQueuedCommand()
        {
            ...
            Debug.Console(2, Parent, "[SendNextQueuedCommand] - Command Queue {0} in progress.",
                CommandQueueInProgress ? "is" : "is not");

            if (!Parent.Communication.IsConnected)
            {
                Debug.Console(2, Parent,
                    "[SendNextQueuedCommand] - Unable to send queued command - Tesira Disconnected");
                return;
            }
            ...
            if (LocalQueue.Peek() is QueuedCommand)
            {
                ...
                Debug.Console(2, Parent, "[SendNextQueuedCommand(QueuedCommand)] - Sending Line - {0}",
                    LastDequeued.Command);
            ...
            {
                LastDequeued = null;
                string nextCommand = (string)LocalQueue.Dequeue();
