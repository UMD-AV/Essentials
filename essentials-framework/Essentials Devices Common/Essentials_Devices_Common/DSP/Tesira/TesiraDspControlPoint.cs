...
namespace Tesira_DSP_EPI
{
    public abstract class TesiraDspControlPoint : EssentialsBridgeableDevice, ISubscribedComponent
    {
        ...
        protected TesiraDspControlPoint(string instanceTag1, string instanceTag2, int index1, int index2,
            TesiraDsp parent, string key, string name, uint? bridgeIndex)
            : base(key, name)
        {
            if (bridgeIndex != null)
        ...
        protected TesiraDspControlPoint(string instanceTag1, string instanceTag2, TesiraDsp parent, string key,
            string name, uint? bridgeIndex)
            : base(key, name)
        {
            if (bridgeIndex != null)
                BridgeIndex = bridgeIndex;
        ...
        public virtual void SendFullCommand(string command, string attributeCode, string value, int instanceTag)
        {
            if (string.IsNullOrEmpty(attributeCode))
            {
                Debug.Console(2, this, Debug.ErrorLogLevel.Error,
                    "SendFullCommand({0}, {1}, {2}, {3}) Error: AttributeCode is null or empty", command, attributeCode,
                    value, instanceTag);
                return;
            }
            ...
            else if (attributeCode == "input")
            {
                ...
                cmd = string.IsNullOrEmpty(value)
                    ? string.Format("\"{0}\" {1} {2} {3}",
                        instanceTagLocal, command, attributeCode,
                        Index1 == 0 ? "" : Index1.ToString(CultureInfo.InvariantCulture))
                    : string.Format("\"{0}\" {1} {2} {3} {4}",
                        instanceTagLocal, command, attributeCode,
                        Index1 == 0 ? "" : Index1.ToString(CultureInfo.InvariantCulture), value);
            }
                     ...
                     attributeCode == "offHook" || attributeCode == "answer")
            {
                ...
                cmd = string.IsNullOrEmpty(value)
                    ? string.Format("\"{0}\" {1} {2} {3}",
                        instanceTagLocal, attributeCode, Index1, Index2)
                    : string.Format("\"{0}\" {1} {2} {3} {4}",
            ...
            int instanceTag)
        {
            ...
            if (string.IsNullOrEmpty(customName) || string.IsNullOrEmpty(attributeCode))
            {
                Debug.Console(2, this,
                    "SendSubscriptionCommand({0}, {1}, {2}, {3}) Error: CustomName or AttributeCode are null or empty",
                    customName, attributeCode, responseRate, instanceTag);
                return;
            }
        ...
        public virtual void SendUnSubscriptionCommand(string customName, string attributeCode, int instanceTag)
        {
            ...
            {
                Debug.Console(2, this,
                    "SendUnSubscriptionCommand({0}, {1}, {2}) Error: CustomName or AttributeCode are null or empty",
                    customName, attributeCode, instanceTag);
                return;
            }
