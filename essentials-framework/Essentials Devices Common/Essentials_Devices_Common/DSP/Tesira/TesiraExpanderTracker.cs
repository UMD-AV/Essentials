...
namespace Tesira_DSP_EPI
{
    public class TesiraExpanderTracker : TesiraDspControlPoint, ICommunicationMonitor
    {
            ...
            : base("", "", 0, 0, parent, string.Format(KeyFormatter, parent.Key, "Expanders"), "Expanders", 0)
        {
            CommunicationMonitor =
                new GenericCommunicationMonitor(this, Parent.Communication, 45000, 90000, 180000, CheckTracker);

            foreach (KeyValuePair<string, TesiraExpanderBlockConfig> tesiraExpanderBlockConfig in expanders)
            {
                int key = tesiraExpanderBlockConfig.Value.Index;
                string value = tesiraExpanderBlockConfig.Value.Hostname;
                TesiraExpanderData expander = new TesiraExpanderData(value, key, this);
                Expanders.Add(expander);
                StringFeedback h = new StringFeedback(() => expander.Hostname);
                StringFeedback s = new StringFeedback(() => expander.SerialNumber);
                StringFeedback f = new StringFeedback(() => expander.Firmware);
                StringFeedback m = new StringFeedback(() => expander.MacAddress);
                BoolFeedback o = new BoolFeedback(() => expander.Online);
        ...
        public override void ParseGetMessage(string attributeCode, string message)
        {
            ...
            */
            ...
            for (int v = 0; v < matches.Count; v++)
            {
                ...
                MatchCollection matchesEnclosed = Regex.Matches(matches[v].ToString(), pattern2);
                string data2 = Regex.Replace(matches[v].ToString(), pattern2, "").Trim('"').Trim('[').Trim()
                    .Replace("  ", " ");
                Console.WriteLine("Data2 = {0}", data2);
                string hostname = matchesEnclosed[0].ToString().Trim('"');

                Debug.Console(2, this, "Match {0} Hostname : {1}", v, hostname);

                TesiraExpanderData newData = Expanders.FirstOrDefault(o =>
                    string.Equals(o.Hostname, hostname, StringComparison.CurrentCultureIgnoreCase));
            ...
            if (Debug.Level != 2) return;
            foreach (TesiraExpanderData device in Expanders)
            {
                TesiraExpanderData i = device;
        ...
        {
            TesiraExpanderJoinMap expanderJoinMap = new TesiraExpanderJoinMap(joinStart);
            ...
            foreach (TesiraExpanderData item in Expanders)
            {
                ...
                Hostnames[index]
                    .LinkInputSig(trilist.StringInput[(uint)(expanderJoinMap.Hostname.JoinNumber + offset)]);
                SerialNumbers[index]
                    .LinkInputSig(trilist.StringInput[(uint)(expanderJoinMap.SerialNumber.JoinNumber + offset)]);
                Firmwares[index]
                    .LinkInputSig(trilist.StringInput[(uint)(expanderJoinMap.Firmware.JoinNumber + offset)]);
                MacAddresses[index]
                    .LinkInputSig(trilist.StringInput[(uint)(expanderJoinMap.MacAddress.JoinNumber + offset)]);
                OnlineStatuses[index]
                    .LinkInputSig(trilist.BooleanInput[(uint)(expanderJoinMap.IsOnline.JoinNumber + offset)]);
            }
    ...
    public class TesiraExpanderData : IDeviceInfoProvider, IKeyName
    {
        ...
        public void SetData(string data)
        {
            ...
            string data2 = Regex.Replace(data, Pattern, "").Trim('"').Trim('[').Trim().Replace("  ", " ");
            Console.WriteLine("Data2 = {0}", data2);
            string[] fData = data2.Split(' ');
