...
namespace Tesira_DSP_EPI
{
    public class TesiraDspRouter : TesiraDspControlPoint, IRoutingWithFeedback
    {
        ...
        public TesiraDspRouter(string key, TesiraRouterControlBlockConfig config, TesiraDsp parent)
            : base(config.RouterInstanceTag, string.Empty, config.Index1, 0, parent,
                string.Format(KeyFormatter, parent.Key, key), config.Label, config.BridgeIndex)
        {
            ...
            foreach (KeyValuePair<uint, RoutingPort> input in config.RouterInputs)
            {
        ...
        private void Initialize(TesiraRouterControlBlockConfig config)
        {
            ...
            RoutingPort output = config.RouterOutput;
        ...
        public void GetSourceNames()
        {
            ...
            {
                RoutingInputPort input = port;
                ushort index = Convert.ToUInt16(input.Selector);
        ...
        {
            TesiraSwitcherJoinMapAdvancedStandalone joinMap = new TesiraSwitcherJoinMapAdvancedStandalone(joinStart);
            ...
            IRoutingWithFeedback s = this as IRoutingWithFeedback;
            ...
            if (ShowRoutedString)
                RoutedSourceNameFeedback.LinkInputSig(trilist.StringInput[joinMap.RouteOrSource.JoinNumber]);
