...
namespace Tesira_DSP_EPI.Extensions
{
    public static class XSigHelper
    {
        ...
        public static string CreateByteString(int index, string value)
        {
            ...
            string indexBinary = (indexHeader + Convert.ToString(index, toBase).PadLeft(10, '0')).Insert(8, "0");

            byte[] array01 = (BitConverter.GetBytes(Convert.ToInt16(indexBinary, 2))).Reverse().ToArray();
            byte[] array02 = Encoding.GetEncoding(28591).GetBytes(value);
            byte[] array03 = { delimiter };

            byte[] dataArray = array01.Concat(array02).Concat(array03).ToArray();
        ...
        public static string CreateByteString(int index, bool value)
        {
            ...
            if (index >= maxIndex) return xSigStr;
            string indexBinary = (Convert.ToString(index, toBase).PadLeft(12, '0')).Insert(5, "0");

            int myData = value ? 0 : 32; //Sets the bit for digital - 0 == high and 1 == low
            myData += 128;
            myData = myData << 8;
        ...
        public static string CreateByteString(int index, int value)
        {
            ...
            if (index >= maxIndex) return xSigStr;
            string binaryValue = ((Convert.ToString(value, toBase).PadLeft(16, '0')).Insert(2, "00")).Insert(11, "0");
            string binaryIndex = (Convert.ToString(index, toBase).PadLeft(10, '0')).Insert(3, "0");

            string dataPacket = ("11" + binaryValue.Insert(3, binaryIndex));

            byte[] dataArray = BitConverter.GetBytes(Convert.ToInt32(dataPacket, 2));
        ...
        {
            XSigData myReturn = new XSigData();

            const int stringByte = 200;
            const int intByte = 192;
            const int boolByte = 128;

            byte[] dataArray = Encoding.GetEncoding(28591).GetBytes(data);
            ...
            int headerByte = (int)dataArray[0];
            ...
            if ((headerByte & stringByte) == stringByte)
            {
                ...
                int index = (dataArray[0] << 8) + dataArray[1];


                string myIndex = Convert.ToString(index, 2).PadLeft(16, '0');
                myIndex = myIndex.Remove(0, 5).Remove(3, 1);
                myReturn.Index = Convert.ToInt16(myIndex, 2);

                myReturn.XString = dataArray.Length > 3
                    ? Encoding.GetEncoding(28591).GetString(dataArray, 2, dataArray.Length - 3)
                    : "";
            }
            ...
            else if ((headerByte & intByte) == intByte)
            {
                ...
                int supplement = (dataArray[2] << 8) + dataArray[3];

                string myIndex = Convert.ToString(index, 2).PadLeft(16, '0');
                string newIndex = myIndex.Remove(0, 5).Remove(3, 1);

                myReturn.Index = Convert.ToInt16(newIndex, 2);

                string mySupplement = Convert.ToString(supplement, 2).PadLeft(16, '0');

                string fullPacket = myIndex + mySupplement;
                string myAnalog = fullPacket.Remove(0, 2).Remove(2, 13).Remove(9, 1);
    ...
    public class XSigData
    {
        ...
        {
            return string.Format("index: {0} type: {1} xString: {2} xInt: {3}, xBool:{4}", Index, SigType, XString,
                XInt,
                XBool);
        }
