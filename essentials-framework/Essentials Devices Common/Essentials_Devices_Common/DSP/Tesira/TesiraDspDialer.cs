    ...
    public class TesiraDspDialer : TesiraDspDialerControlPoint, IBridgeAdvanced
    {
        ...
        public ECallStatus CallStatusEnum
        {
            ...
            protected set
            {
                ...
                {
                    OffHookStatus = true;
                    string cidCmd = IsVoip ? string.Format("cid {0} {1}", Index1, CallAppearance) : "cid";
        ...
        {
            get { return CallStatusEnum == ECallStatus.RINGING; }
        }
        ...
        public TesiraDspDialer(string key, TesiraDialerControlBlockConfig config, TesiraDsp parent)
            : base(key, config.DialerInstanceTag, config.ControlStatusInstanceTag, config.Index, config.CallAppearance,
                parent, config.BridgeIndex)
        {
            ...
            try
            {
                ...
                if (customName == ControlStatusCustomName || customName == PotsDialerCustomName)
                {
                    ...
                    const string pattern2 =
                        "\\[(?<state>\\d+)\\s+(?<line>\\d+)\\s+(?<call>\\d+)\\s+(?<action>\\d+)\\s+(?<cid>\".+\"|\"\")\\s+(?<prompt>\\d+)\\]";
                    //Pulls CallerID Data
                    const string pattern3 =
                        "(?:(?:\\\\\"(?<time>.*)\\\\\")(?:\\\\\"(?<number>.*)\\\\\")(?:\\\\\"(?<name>.*)\\\\\"))|\"\"";

                    MatchCollection myMatches = Regex.Matches(value, pattern1);
                    ...
                    if (match2.Success)
                    {
                        ...
                        int lineNumber = ushort.Parse(match2.Groups["line"].Value) + 1;
                        ...
                        else
                        {
                            ...
                            {
                                case 1:
                                    CallStatusEnum = ECallStatus.IDLE;
                                    break;
                                case 2:
                                    CallStatusEnum = ECallStatus.DIALING;
                                    break;
                                ...
                                case 4:
                                    ...
                                    break;
                                case 5:
                                    CallStatusEnum = ECallStatus.REJECT;
                                    break;
                                case 6:
                                    CallStatusEnum = ECallStatus.ACTIVE;
                                    break;
                                case 7:
                                    CallStatusEnum = ECallStatus.RINGING;
                                    break;
                                case 8:
                                    CallStatusEnum = ECallStatus.REJECT;
                                    break;
                                case 12:
                                    CallStatusEnum = ECallStatus.INIT;
                                    break;
                                case 13:
                                    CallStatusEnum = ECallStatus.FAULT;
                                    break;
                                case 14:
                                    CallStatusEnum = ECallStatus.SILENT;
                                    break;
...
// ReSharper restore InconsistentNaming
        ...
        {
            TesiraDialerJoinMapAdvancedStandalone joinMap = new TesiraDialerJoinMapAdvancedStandalone(joinStart);
