...
namespace ExtronDmp
{
    ...
    public class ExtronDmp : EssentialsBridgeableDevice, ICommunicationMonitor, IOnline
    {
        ...
        void InitializeDspObjects()
        {
            ...
            foreach (KeyValuePair<int, ExtronDmpLevelControl> channel in LevelControlPoints)
            {
        ...
        {
            ExtronDmpJoinMap joinMap = new ExtronDmpJoinMap(joinStart);
            ...
            foreach (KeyValuePair<int, ExtronDmpLevelControl> channel in LevelControlPoints)
            {
                ...
                trilist.StringInput[joinMap.ChannelName.JoinNumber + channelIndex].StringValue =
                    channel.Value.LevelCustomName;
                trilist.UShortInput[joinMap.ChannelType.JoinNumber + channelIndex].UShortValue =
                    (ushort)channel.Value.Type;
                ...
                genericChannel.MuteFeedback.LinkInputSig(
                    trilist.BooleanInput[joinMap.ChannelMuteToggle.JoinNumber + channelIndex]);
                genericChannel.MuteFeedback.LinkComplementInputSig(
                    trilist.BooleanInput[joinMap.ChannelMuteOff.JoinNumber + channelIndex]);
                genericChannel.VolumeLevelFeedback.LinkInputSig(
                    trilist.UShortInput[joinMap.ChannelVolume.JoinNumber + channelIndex]);
                // from SiMPL > to Plugin
                trilist.SetSigTrueAction(joinMap.ChannelMuteToggle.JoinNumber + channelIndex,
                    () => genericChannel.MuteToggle());
                trilist.SetSigTrueAction(joinMap.ChannelMuteOn.JoinNumber + channelIndex,
                    () => genericChannel.MuteOn());
                trilist.SetSigTrueAction(joinMap.ChannelMuteOff.JoinNumber + channelIndex,
                    () => genericChannel.MuteOff());
                // from SiMPL > to Plugin
                trilist.SetBoolSigAction(joinMap.ChannelVolumeUp.JoinNumber + channelIndex,
                    b => genericChannel.VolumeUp(b));
                trilist.SetBoolSigAction(joinMap.ChannelVolumeDown.JoinNumber + channelIndex,
                ...
                trilist.SetSigFalseAction(joinMap.EnableLevelSend.JoinNumber + channelIndex, () =>
                {
                    ...
                    genericChannel.SetVolume(trilist.UShortOutput[joinMap.ChannelVolume.JoinNumber + channelIndex]
                        .UShortValue);
                });
            ...
            foreach (KeyValuePair<ushort, ExtronDmpDialer> line in Dialers)
            {
                KeyValuePair<ushort, ExtronDmpDialer> dialer = line;
                ...
                // from SIMPL > to Plugin
                trilist.SetStringSigAction(joinMap.KeyPadDial.JoinNumber + dialerLineOffset,
                    directDialString => dialer.Value.Dial(directDialString));
                ...
                dialer.Value.CallerIdNumberFeedback.LinkInputSig(
                    trilist.StringInput[joinMap.CallerIdNumberFb.JoinNumber + dialerLineOffset]);

                // from Plugin > to SiMPL
                dialer.Value.OffHookFeedback.LinkInputSig(
                    trilist.BooleanInput[joinMap.KeyPadDial.JoinNumber + dialerLineOffset]);
                dialer.Value.OffHookFeedback.LinkInputSig(
                ...
                dialer.Value.DialStringFeedback.LinkInputSig(
                    trilist.StringInput[joinMap.DialString.JoinNumber + dialerLineOffset]);
            ...
            foreach (ExtronDmpPreset preset in PresetList)
            {
                ushort x = preset.id;
        ...
        {
            get { return _commMonitor.IsOnlineFeedback; }
        }
