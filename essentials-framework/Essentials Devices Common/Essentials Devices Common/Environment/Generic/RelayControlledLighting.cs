...
namespace PepperDash.Essentials.Devices.Common.Environment
{
    public class RelayControlledLighting : LightingBase
    {
       ...
       private Relay GetRelay(string portDeviceKey, uint portNumber)
       {
           ...
           if (portDeviceKey.Equals("processor"))
           {
               ...
               relayDevice.RelayPorts[portNumber].StateChange +=new RelayEventHandler(RelayControlledLighting_StateChange);
               return relayDevice.RelayPorts[portNumber];
           }

           var essentialsDevice = DeviceManager.GetDeviceForKey(portDeviceKey);
           if (essentialsDevice == null)
           {
               Debug.Console(0, "Device {0} was not found in Device Manager. Check configuration or for errors with device.", portDeviceKey);
               return null;
           }
            ...
            CrestronInvoke.BeginInvoke((o) =>
                {
                    if (LightingScenes != null && LightingScenes[sceneNum] != null)
                    {
                        ...
                        if (test)
                        {
                            try
                            {
                                var scene = LightingScenes[sceneNum];
    ...
    public class RelayControlledLightingFactory : EssentialsDeviceFactory<RelayControlledLighting>
    {
        ...
        public override EssentialsDevice BuildDevice(DeviceConfig dc)
        {
            ...
            var props = Newtonsoft.Json.JsonConvert.DeserializeObject<RelayControlledLightingPropertiesConfig>(dc.Properties.ToString());
