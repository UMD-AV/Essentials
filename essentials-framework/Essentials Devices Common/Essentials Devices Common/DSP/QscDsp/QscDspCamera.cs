...
namespace QscQsysDspPlugin
{
    ...
    public class QscDspCamera : EssentialsBridgeableDevice, IOnline
	{
		...
		{
            var joinMap = new QscDspCameraDeviceJoinMap(joinStart);
            ...
            trilist.SetBoolSigAction(joinMap.Up.JoinNumber, (b) => this.MoveCamera(b ? eCameraPtzControls.TiltUp : eCameraPtzControls.Stop));
            trilist.SetBoolSigAction(joinMap.Down.JoinNumber, (b) => this.MoveCamera(b ? eCameraPtzControls.TiltDown : eCameraPtzControls.Stop));
            trilist.SetBoolSigAction(joinMap.Left.JoinNumber, (b) => this.MoveCamera(b ? eCameraPtzControls.PanLeft : eCameraPtzControls.Stop));
            trilist.SetBoolSigAction(joinMap.Right.JoinNumber, (b) => this.MoveCamera(b ? eCameraPtzControls.PanRight : eCameraPtzControls.Stop));
            trilist.SetBoolSigAction(joinMap.ZoomIn.JoinNumber, (b) => this.MoveCamera(b ? eCameraPtzControls.ZoomIn : eCameraPtzControls.Stop));
            trilist.SetBoolSigAction(joinMap.ZoomOut.JoinNumber, (b) => this.MoveCamera(b ? eCameraPtzControls.ZoomOut : eCameraPtzControls.Stop));
            ...
            foreach (var preset in this.Config.Presets)
            {
                var temp = x;
                // from SiMPL > to Plugin
                trilist.SetSigTrueAction(joinMap.PresetRecallStart.JoinNumber + temp + 1, () => this.RecallPreset(temp));
