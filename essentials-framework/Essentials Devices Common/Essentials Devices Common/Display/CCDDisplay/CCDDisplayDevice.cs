...
namespace CCDDisplay
{
    ...
    public class CCDDisplayDevice : TwoWayDisplayBase, IBasicVolumeWithFeedback, ICommunicationMonitor, IBridgeAdvanced
    {
        ...
        private int _inputFeedbackIndex;
        private IBasicVideoDisplay _display;
        public Dictionary<uint, BoolFeedback> InputFeedbacks { get; private set; }
            ...
            : base(key, name)
        {
            ...
            LampHours1Feedback = new IntFeedback(() => _display.SupportsLampHours ? (int)_display.LampHours[0] : 0);
            Feedbacks.Add(LampHours1Feedback);

            InputFeedbacks = new Dictionary<uint, BoolFeedback>();
        ...
        private void AddInput(uint index)
        {
            uint myIndex = index;
            var boolFeedback = new BoolFeedback(() => {
                return myIndex == _inputFeedbackIndex;                
            });

            InputFeedbacks.Add(index, boolFeedback);                                
            Feedbacks.Add(boolFeedback);
        }
        ...
        private void displayStateChangeEvent(DisplayStateObjects state, IBasicVideoDisplay display, byte arg3)
        {
            switch (state)
            {
                ...
                case DisplayStateObjects.Input:
                    ...
                    _inputFeedbackIndex = InputPorts.IndexOf(newInputPort);
                    Debug.Console(1, "Input feedback: {0}",_inputFeedbackIndex);
                    UpdateInputFeedbacks();
        ...
        public void LinkToApi(BasicTriList trilist, uint joinStart, string joinMapKey, EiscApiAdvanced bridge)
        {
            ...
            foreach (var i in InputFeedbacks)
            {
                i.Value.LinkInputSig(trilist.BooleanInput[joinMap.InputSelectOffset.JoinNumber + i.Key]);
            }
        ...
        private void UpdateInputFeedbacks()
        {
            foreach (var i in InputFeedbacks)
            {
                Debug.Console(1, "Updating input feedback bool for key: {0}", i.Key);
                i.Value.FireUpdate();
            }
        }
