...
namespace PepperDash.Essentials.Devices.Common.ImageProcessors
{
    public class AutomateVx : EssentialsDevice, IBridgeAdvanced
    {
        ...
        public void LinkToApi(BasicTriList trilist, uint joinStart, string joinMapKey, EiscApiAdvanced bridge)
        {
            ...
            LayoutNamesUpdated += (o, a) =>
            {
                ...
                {
                    uint count = 0;
                    foreach (var entry in LayoutNames)
                    {
        ...
        void UpdateLayoutFeedback(string layout)
        {
            if (LayoutNames != null)
            {
                var layoutFb = LayoutNames.FirstOrDefault(o => o.Id == layout);
                if (layoutFb != null)
                {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            Debug.Console(1, this, "Http client response content:{0}", response.ContentString.ToString());
                            JObject obj = JObject.Parse(response.ContentString);
                            var status = obj["status"];
                            if ((string)status == "OK")
                            {
                            ...
                            {
                                Debug.Console(0, this, "Http client JSON returned and error: {0}", (string)obj["error"]);
                            }
                            else
                            {
    ...
    public class AutomateVxFactory : EssentialsDeviceFactory<AutomateVx>
    {
        ...
        public override EssentialsDevice BuildDevice(DeviceConfig dc)
        {
            ...
            var props = Newtonsoft.Json.JsonConvert.DeserializeObject<AutomateVxPropertiesConfig>(
    ...
    public class AutomateVxJoinMap : JoinMapBaseAdvanced
    {
        ...
        [JoinName("CloseWirecast")]
        public JoinDataComplete CloseWirecast = new JoinDataComplete(
            new JoinData()
            {
                JoinNumber = 6,
                JoinSpan = 1
        ...
        [JoinName("LayoutRecall")]
        public JoinDataComplete LayoutRecall = new JoinDataComplete(
            new JoinData()
            {
                JoinNumber = 2,
                JoinSpan = 1
        ...
        [JoinName("LayoutIds")]
        public JoinDataComplete LayoutIds = new JoinDataComplete(
            new JoinData()
            {
                JoinNumber = 20,
                JoinSpan = 10
