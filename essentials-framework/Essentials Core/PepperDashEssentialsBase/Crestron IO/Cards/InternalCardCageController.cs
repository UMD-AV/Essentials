...
namespace PepperDash.Essentials.Core.CrestronIO.Cards
{
    ...
    public class InternalCardCageController : EssentialsDevice
    {
        ...
        public InternalCardCageController(string key, string name, InternalCardCageConfiguration config) : base(key,
            name)
        {
            _config = config;
        ...
        private void GetCards()
        {
            ...
            for (uint i = 1; i <= CardSlots; i++)
            {
                ...
                try
                {
                    C3CardControllerBase device = cardBuilder(i);
        ...
        public override EssentialsDevice BuildDevice(DeviceConfig dc)
        {
            ...
            if (!Global.ControlSystem.SupportsThreeSeriesPlugInCards)
            {
                Debug.Console(0, Debug.ErrorLogLevel.Warning,
                    "Current control system does NOT support 3-Series cards. Everything is NOT awesome.");
                return null;
            }

            InternalCardCageConfiguration config = dc.Properties.ToObject<InternalCardCageConfiguration>();
