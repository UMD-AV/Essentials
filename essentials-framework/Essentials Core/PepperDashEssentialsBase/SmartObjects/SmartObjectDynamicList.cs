...
namespace PepperDash.Essentials.Core.SmartObjects
{
    public class SmartObjectDynamicList : SmartObjectHelperBase
    {
        ...
        public SmartObjectDynamicList(SmartObject so, bool useUserObjectHandler, uint nameSigOffset) : base(so,
            useUserObjectHandler)
        {
            try
            {
            ...
            catch
            {
                string msg = string.Format(
                    "SmartObjectDynamicList: Smart Object {0:X2}-{1} is not a dynamic list. Ignoring", so.Device.ID,
                    so.ID);
