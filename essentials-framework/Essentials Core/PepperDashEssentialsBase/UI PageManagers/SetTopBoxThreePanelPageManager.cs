...
namespace PepperDash.Essentials.Core.PageManagers
{
    public class ThreePanelPlusOnePageManager : PageManager
    {
        ...
        public override void Show()
        {
            ...
            List<BoolInputSig> fixedSigs = FixedVisibilityJoins.Select(u => TriList.BooleanInput[u]).ToList();
            foreach (BoolInputSig sig in fixedSigs)
                sig.BoolValue = true;
            ...
            if (ShowPosition5Tabs)
            {
                ...
                // hook up tab object
                SmartObject tabSo = TriList.SmartObjects[Position5TabsId];
                tabSo.BooleanOutput["Tab Button 1 Press"].UserObject = new Action<bool>(b =>
                {
                    if (!b) ShowTab(1);
                });
                tabSo.BooleanOutput["Tab Button 2 Press"].UserObject = new Action<bool>(b =>
                {
                    if (!b) ShowTab(2);
                });
        ...
        private void ShowTab(uint number)
        {
            ...
            // Swap subpage
            DeviceBooleanInputCollection bi = TriList.BooleanInput;
    ...
    public class SetTopBoxThreePanelPageManager : ThreePanelPlusOnePageManager
    {
            ...
            : base(trilist)
        {
            ...
            if (dpad && !preset && !dvr && !numbers) FixedVisibilityJoins = new uint[] { 10031, 10091 };
                ...
                FixedVisibilityJoins = new uint[] { 10053, 10032, 10011, 10093 };
            else if (dpad && preset && !dvr && numbers)
                FixedVisibilityJoins = new uint[] { 10054, 10032, 10011, 10093 };
            else if (dpad && !preset && dvr && numbers)
                FixedVisibilityJoins = new uint[] { 10054, 10033, 10011, 10093 };
            else if (!dpad && preset && dvr && numbers)
                FixedVisibilityJoins = new uint[] { 10054, 10033, 10012, 10093 };
