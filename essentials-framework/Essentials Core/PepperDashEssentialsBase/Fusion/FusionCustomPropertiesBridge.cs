        ...
        public void EvaluateRoomInfo(string roomKey, RoomInformation roomInfo)
        {
            try
            {
                ...
                foreach (IKeyed device in reconfigurableDevices)
                {
                    ...
                    DeviceConfig deviceConfig = (device as ReconfigurableDevice).Config;

                    if (device is RoomOnToDefaultSourceWhenOccupied)
                    {
                        Debug.Console(1, "Mapping Room on via Occupancy values from Fusion");

                        RoomOnToDefaultSourceWhenOccupiedConfig devProps =
                            JsonConvert.DeserializeObject<RoomOnToDefaultSourceWhenOccupiedConfig>(
                                deviceConfig.Properties.ToString());

                        FusionCustomProperty enableFeature =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupied"));
                        if (enableFeature != null)
                            devProps.EnableRoomOnWhenOccupied = bool.Parse(enableFeature.CustomFieldValue);

                        FusionCustomProperty enableTime =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p =>
                                p.ID.Equals("RoomOnWhenOccupiedStartTime"));
                        if (enableTime != null)
                            devProps.OccupancyStartTime = enableTime.CustomFieldValue;

                        FusionCustomProperty disableTime =
                            roomInfo.FusionCustomProperties.FirstOrDefault(
                                p => p.ID.Equals("RoomOnWhenOccupiedEndTime"));
                        if (disableTime != null)
                            devProps.OccupancyEndTime = disableTime.CustomFieldValue;

                        FusionCustomProperty enableSunday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedSun"));
                        if (enableSunday != null)
                            devProps.EnableSunday = bool.Parse(enableSunday.CustomFieldValue);

                        FusionCustomProperty enableMonday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedMon"));
                        if (enableMonday != null)
                            devProps.EnableMonday = bool.Parse(enableMonday.CustomFieldValue);

                        FusionCustomProperty enableTuesday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedTue"));
                        if (enableTuesday != null)
                            devProps.EnableTuesday = bool.Parse(enableTuesday.CustomFieldValue);

                        FusionCustomProperty enableWednesday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedWed"));
                        if (enableWednesday != null)
                            devProps.EnableWednesday = bool.Parse(enableWednesday.CustomFieldValue);

                        FusionCustomProperty enableThursday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedThu"));
                        if (enableThursday != null)
                            devProps.EnableThursday = bool.Parse(enableThursday.CustomFieldValue);

                        FusionCustomProperty enableFriday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedFri"));
                        if (enableFriday != null)
                            devProps.EnableFriday = bool.Parse(enableFriday.CustomFieldValue);

                        FusionCustomProperty enableSaturday =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("EnRoomOnWhenOccupiedSat"));
                    ...
                    else if (device is IEssentialsRoom)
                    {
                        ...
                        {
                            Debug.Console(1, "Current Room Name: {0}. New Room Name: {1}", deviceConfig.Name,
                                roomInfo.Name);
                        ...
                        FusionCustomProperty helpMessage =
                            roomInfo.FusionCustomProperties.FirstOrDefault(p => p.ID.Equals("RoomHelpMessage"));
                        if (helpMessage != null)
                        {
