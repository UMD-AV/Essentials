...
namespace PepperDash.Essentials.Core
{
    public static class DeviceManager
    {
        ...
        /// </summary>
        public static List<IKeyed> AllDevices
        {
            get { return new List<IKeyed>(Devices.Values); }
        }
        ...
        public static void Initialize(CrestronControlSystem cs)
        {
            ...
            CrestronConsole.AddNewConsoleCommand(ListDeviceCommStatuses, "devcommstatus",
                "Lists the communication status of all devices",
            ...
            CrestronConsole.AddNewConsoleCommand(
                s => CrestronConsole.ConsoleCommandResponse(DeviceJsonApi.GetProperties(s)), "devprops", "",
                ConsoleAccessLevelEnum.AccessOperator);
            CrestronConsole.AddNewConsoleCommand(
                s => CrestronConsole.ConsoleCommandResponse(DeviceJsonApi.GetMethods(s)), "devmethods", "",
                ConsoleAccessLevelEnum.AccessOperator);
            CrestronConsole.AddNewConsoleCommand(
                s => CrestronConsole.ConsoleCommandResponse(DeviceJsonApi.GetApiMethods(s)), "apimethods", "",
                ConsoleAccessLevelEnum.AccessOperator);
            CrestronConsole.AddNewConsoleCommand(SimulateComReceiveOnDevice, "devsimreceive",
                "Simulates incoming data on a com device", ConsoleAccessLevelEnum.AccessOperator);

            CrestronConsole.AddNewConsoleCommand(SetDeviceStreamDebugging, "setdevicestreamdebug",
                "set comm debug [deviceKey] [off/rx/tx/both] ([minutes])", ConsoleAccessLevelEnum.AccessOperator);
            CrestronConsole.AddNewConsoleCommand(s => DisableAllDeviceStreamDebugging(), "disableallstreamdebug",
                "disables stream debugging on all devices", ConsoleAccessLevelEnum.AccessOperator);
        }
        ...
        private static void ListDevices(string s)
        {
            ...
            List<IKeyed> sorted = Devices.Values.ToList();
            sorted.Sort((a, b) => a.Key.CompareTo(b.Key));

            foreach (IKeyed d in sorted)
            {
                string name = d is IKeyName ? (d as IKeyName).Name : "---";
        ...
        {
            IKeyed dev = GetDeviceForKey(devKey);
            if (dev == null)
            {
            ...
            IHasFeedback statusDev = dev as IHasFeedback;
            if (statusDev == null)
            {
    ...
    timeOutInMinutes - Set timeout for stream debugging. Default is 30 minutes");
            ...
            if (!string.IsNullOrEmpty(timeout))
            {
                try
                {
                    uint min = Convert.ToUInt32(timeout);

                    device.StreamDebugging.SetDebuggingWithSpecificTimeout(debugSetting, min);
                    Debug.Console(0, "Device: '{0}' debug level set to {1} for {2} minutes", deviceKey, debugSetting,
                        min);
                }
                catch (Exception e)
                {
                    Debug.Console(0,
                        "Unable to convert minutes or settings value.  Please use an integer value for minutes. Errro: {0}",
                        e);
                }
            }
            else
            {
                ...
                Debug.Console(0, "Device: '{0}' debug level set to {1} for default time (30 minutes)", deviceKey,
                    debugSetting);
            }
        }
        ...
        {
            foreach (IKeyed device in AllDevices)
            {
