...
namespace PepperDash.Essentials.Core
{
    public class DeviceJsonApi
    {
        ...
        public static void DoDeviceActionWithJson(string json)
        {
            ...
            try
            {
                DeviceActionWrapper action = JsonConvert.DeserializeObject<DeviceActionWrapper>(json);
            ...
            {
                CrestronConsole.ConsoleCommandResponse(
                    "Incorrect format for JSON. Please check that the format matches {\"deviceKey\":\"myDevice\", \"methodName\":\"someMethod\", \"params\": [\"param1\", true]}");
            }
        }
        ...
        {
            string key = action.DeviceKey;
            object obj = FindObjectOnPath(key);
            if (obj == null)
            {
            ...
            try
            {
                List<MethodInfo> methods = t.GetMethods().Where(m => m.Name == action.MethodName).ToList();

                MethodInfo method = methods.Count == 1
                    ? methods[0]
                    : methods.FirstOrDefault(m => m.GetParameters().Length == action.Params.Length);
        ...
        {
            object obj = FindObjectOnPath(deviceObjectPath);
            if (obj == null)
                return "{ \"error\":\"No Device\"}";

            CType t = obj.GetType();
            // get the properties and set them into a new collection of NameType wrappers
            IEnumerable<PropertyNameType> props = t.GetProperties().Select(p => new PropertyNameType(p, obj));
        ...
        {
            object dev = FindObjectOnPath(deviceObjectPath);
            if (dev == null)
                return "{ \"error\":\"No Device\"}";
            ...
            {
                Debug.Console(1, "Unable to find Property: {0} on Device with path: {1}", propertyName,
                    deviceObjectPath);
                return null;
            }
        }
        ...
        public static string GetMethods(string deviceObjectPath)
        {
            ...
            CType t = obj.GetType();
            IEnumerable<MethodNameParams> methods = t.GetMethods()
        ...
        {
            string[] path = deviceObjectPath.Split('.');

            IKeyed dev = DeviceManager.GetDeviceForKey(path[0]);
            if (dev == null)
            {
            ...
            if (path.Length > 1)
            {
                for (int i = 1; i < path.Length; i++)
                {
                    ...
                    int indexOpen = objName.IndexOf('[');
                    if (indexOpen != -1)
                    {
                        int indexClose = objName.IndexOf(']');
                        if (indexClose == -1)
                        {
                    ...
                    PropertyInfo prop = oType.GetProperty(objName);
                    if (prop == null)
                    {
                    ...
                    if (indexStr != null)
                    {
                        ...
                        PropertyInfo indexedPropInfo = prop.PropertyType.GetProperty("Item");
                        // These are the parameters for the indexing. Only care about one
                        ParameterInfo[] indexParams = indexedPropInfo.GetIndexParameters();
                        if (indexParams.Length > 0)
                        {
                            Debug.Console(0, "  Indexed, param type: {0}", indexParams[0].ParameterType.Name);
                            object properParam = Convert.ChangeType(indexStr, indexParams[0].ParameterType,
    ...
    public class PropertyNameType
    {
        ...
        public string Type
        {
            get { return PropInfo.PropertyType.Name; }
        }
    ...
    {
        [JsonIgnore] public MethodInfo MethodInfo { get; private set; }
