...
namespace PepperDash.Essentials.Core
{
    public class RouteRequest
    {
        ...
        {
            IWarmingCooling coolingDevice = sender as IWarmingCooling;
    ...
    public static class IRoutingInputsExtensions
    {
        ...
        /// </summary>
        public static void ReleaseAndMakeRoute(this IRoutingSink destination, IRoutingOutputs source,
            eRoutingSignalType signalType)
        {
            ...
            IWarmingCooling coolingDevice = destination as IWarmingCooling;

            RouteRequest existingRouteRequest;

            //We already have a route request for this device, and it's a cooling device and is cooling
            if (RouteRequests.TryGetValue(destination.Key, out existingRouteRequest) && coolingDevice != null &&
                coolingDevice.IsCoolingDownFeedback.BoolValue == true)
            {
                ...
                Debug.Console(2,
                    "******************************************************** Device: {0} is cooling down and already has a routing request stored.  Storing new route request to route to source key: {1}",
                    destination.Key, routeRequest.Source.Key);
            ...
            if (coolingDevice != null && coolingDevice.IsCoolingDownFeedback.BoolValue == true)
            {
                ...
                Debug.Console(2,
                    "******************************************************** Device: {0} is cooling down.  Storing route request to route to source key: {1}",
                    destination.Key, routeRequest.Source.Key);
                return;
            }

            if (RouteRequests.ContainsKey(destination.Key) && coolingDevice != null &&
                coolingDevice.IsCoolingDownFeedback.BoolValue == false)
            {
                RouteRequests.Remove(destination.Key);
                Debug.Console(2,
                    "******************************************************** Device: {0} is NOT cooling down.  Removing stored route request and routing to source key: {1}",
                    destination.Key, routeRequest.Source.Key);
            }
        ...
        public static void RunRouteRequest(RouteRequest request)
        {
            ...
            RouteDescriptor newRoute = request.Destination.GetRouteToSource(request.Source, request.SignalType);
        ...
        public static void ReleaseRoute(this IRoutingSink destination)
        {
            ...
            RouteDescriptor current = RouteDescriptorCollection.DefaultCollection.RemoveRouteDescriptor(destination);
            if (current != null)
            {
        ...
        /// </summary>
        public static RouteDescriptor GetRouteToSource(this IRoutingSink destination, IRoutingOutputs source,
            eRoutingSignalType signalType)
        {
            ...
            else
            {
                ...
                bool audioSuccess =
                    destination.GetRouteToSource(source, null, null, eRoutingSignalType.Audio, 0, routeDescr);
                if (!audioSuccess)
                    Debug.Console(1, destination, "Cannot find audio route to {0}", source.Key);
                bool videoSuccess =
                    destination.GetRouteToSource(source, null, null, eRoutingSignalType.Video, 0, routeDescr);
                if (!videoSuccess)
            ...
            eRoutingSignalType signalType, int cycle, RouteDescriptor routeTable)
        {
            ...
            IEnumerable<TieLine> destDevInputTies = TieLineCollection.Default.Where(t =>
                t.DestinationPort.ParentDevice == destination && (t.Type == signalType ||
                                                                  (t.Type & (eRoutingSignalType.Audio |
                ...
                foreach (TieLine inputTieToTry in attachedMidpoints)
                {
                    ...
                    if (alreadyCheckedDevices.Contains(upstreamRoutingDevice))
                    {
                        Debug.Console(2, destination, "Skipping input {0} on {1}, this was already checked",
                            upstreamRoutingDevice.Key, destination.Key);
                        continue;
                    }

                    // haven't seen this device yet.  Do it.  Pass the output port to the next
                    // level to enable switching on success
                    bool upstreamRoutingSuccess = upstreamRoutingDevice.GetRouteToSource(source,
                        upstreamDeviceOutputPort,
    ...
    public class RouteDescriptorCollection
    {
        ...
        {
            RouteDescriptor descr = GetRouteDescriptorForDestination(destination);
            if (descr != null)
    ...
    public class RouteDescriptor
    {
        ...
        public void ExecuteRoutes()
        {
            foreach (RouteSwitchDescriptor route in Routes)
            {
                ...
                {
                    IRoutingSinkWithSwitching device = route.SwitchingDevice as IRoutingSinkWithSwitching;
                    if (device == null)
                        continue;
                ...
                {
                    (route.SwitchingDevice as IRouting).ExecuteSwitch(route.InputPort.Selector,
                        route.OutputPort.Selector, SignalType);
                    route.OutputPort.InUseTracker.AddUser(Destination, "destination-" + SignalType);
                    Debug.Console(2, "Output port {0} routing. Count={1}", route.OutputPort.Key,
        ...
        {
            string[] routesText = Routes.Select(r => r.ToString()).ToArray();
            return string.Format("Route table from {0} to {1}:\r{2}", Source.Key, Destination.Key,
                string.Join("\r", routesText));
        }
    }
    ...
    {
        public IRoutingInputs SwitchingDevice
        {
            get { return InputPort.ParentDevice; }
        }
        ...
        public override string ToString()
        {
            if (SwitchingDevice is IRouting)
                return string.Format("{0} switches output '{1}' to input '{2}'", SwitchingDevice.Key,
                    OutputPort.Selector, InputPort.Selector);
            else
