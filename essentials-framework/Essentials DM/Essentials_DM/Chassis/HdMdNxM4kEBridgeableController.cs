        ...
        IHasFeedback
    {
            ...
            : base(key, name, chassis)
        {
            ...
            for (uint i = 1; i <= _Chassis.NumberOfInputs; i++)
            {
                ...
                VideoInputSyncFeedbacks.Add(new BoolFeedback(inputKey,
                    () => _Chassis.HdmiInputs[index].VideoDetectedFeedback == null
                        ? false
                        : _Chassis.HdmiInputs[index].VideoDetectedFeedback.BoolValue));
                InputNameFeedbacks.Add(new StringFeedback(inputKey,
                    () => _Chassis.HdmiInputs[index].NameFeedback == null
                        ? ""
                        : _Chassis.HdmiInputs[index].NameFeedback.StringValue));
                InputHdcpFeedbacks.Add(new IntFeedback(inputKey,
                    () => _Chassis.HdmiInputs[index].HdmiInputPort.HdcpSupportOnFeedback == null ? 0 :
                        _Chassis.HdmiInputs[index].HdmiInputPort.HdcpSupportOnFeedback.BoolValue ? 1 : 0));
            }
            ...
            for (uint i = 1; i <= _Chassis.NumberOfOutputs; i++)
            {
                ...
                VideoOutputRouteFeedbacks.Add(new IntFeedback(outputKey,
                    () => _Chassis.HdmiOutputs[index].VideoOutFeedback == null
                        ? 0
                        : (int)_Chassis.HdmiOutputs[index].VideoOutFeedback.Number));
                OutputNameFeedbacks.Add(new StringFeedback(outputKey,
                    () => _Chassis.HdmiOutputs[index].NameFeedback == null
                        ? ""
                        : _Chassis.HdmiOutputs[index].NameFeedback.StringValue));
                OutputRouteNameFeedbacks.Add(new StringFeedback(outputKey,
                    () => _Chassis.HdmiOutputs[index].VideoOutFeedback == null
                        ? ""
        ...
        {
            HdMdNxMHdmiInput input = inputSelector as HdMdNxMHdmiInput; //changed from HdMdNxM4kzEHdmiInput;
            HdMdNxMHdmiOutput output = outputSelector as HdMdNxMHdmiOutput;
        ...
        {
            HdMdNxMHdmiInput input = inputSelector == 0 ? null : _Chassis.HdmiInputs[inputSelector];
            HdMdNxMHdmiOutput output = _Chassis.HdmiOutputs[outputSelector];
        ...
        {
            HdMdNxM4kEControllerJoinMap joinMap = new HdMdNxM4kEControllerJoinMap(joinStart);
            ...
            for (uint i = 1; i <= _Chassis.NumberOfInputs; i++)
            {
                ...
                //Digital
                VideoInputSyncFeedbacks[inputKey]
                    .LinkInputSig(trilist.BooleanInput[joinMap.VideoSyncStatus.JoinNumber + joinIndex]);

                //Analog
                trilist.SetUShortSigAction(joinMap.HdcpSupportCapability.JoinNumber + joinIndex,
                    (hdcpLevel) => SetHdcp(input, hdcpLevel));
                InputHdcpFeedbacks[inputKey]
                    .LinkInputSig(trilist.UShortInput[joinMap.HdcpSupportState.JoinNumber + joinIndex]);

                //Serial                
                InputNameFeedbacks[inputKey]
            ...
            for (uint i = 1; i <= _Chassis.NumberOfOutputs; i++)
            {
                ...
                //Analog
                VideoOutputRouteFeedbacks[outputKey]
                    .LinkInputSig(trilist.UShortInput[joinMap.OutputRoute.JoinNumber + joinIndex]);
                ...
                //Serial
                OutputNameFeedbacks[outputKey]
                    .LinkInputSig(trilist.StringInput[joinMap.OutputNames.JoinNumber + joinIndex]);
                OutputRouteNameFeedbacks[outputKey]
                    .LinkInputSig(trilist.StringInput[joinMap.OutputCurrentInputNames.JoinNumber + joinIndex]);
            }
        }
        ...
        void Chassis_DMOutputChange(Switch device, DMOutputEventArgs args)
        {
            switch (args.EventId)
            {
                case DMOutputEventIds.VideoOutEventId:
                {
                    ...
                    uint inputNumber = _Chassis.HdmiOutputs[output].VideoOutFeedback == null
                        ? 0
                    ...
                    RoutingInputPort inPort =
                        InputPorts.FirstOrDefault(p =>
                            p.FeedbackMatchObject == _Chassis.HdmiOutputs[output].VideoOutFeedback);
                    RoutingOutputPort outPort =
                        OutputPorts.FirstOrDefault(p => p.FeedbackMatchObject == _Chassis.HdmiOutputs[output]);
        ...
        public class HdMdNxM4kEControllerFactory : EssentialsDeviceFactory<HdMdNxM4kEBridgeableController>
        {
            ...
            public override EssentialsDevice BuildDevice(DeviceConfig dc)
            {
                ...
                HdMdNxM4kEBridgeablePropertiesConfig props =
                    JsonConvert.DeserializeObject<HdMdNxM4kEBridgeablePropertiesConfig>(dc.Properties.ToString());

                string type = dc.Type.ToLower();
                uint ipid = props.Control.IpIdInt;
                string address = props.Control.TcpSshProperties.Address;
