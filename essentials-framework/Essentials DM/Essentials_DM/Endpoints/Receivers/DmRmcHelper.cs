...
namespace PepperDash.Essentials.DM
{
    ...
    public abstract class DmRmcControllerBase : CrestronGenericBridgeableBaseDevice, IDeviceInfoProvider
    {
        ...
        private readonly EndpointReceiverBase
            _rmc; //kept here just in case. Only property or method on this class that's not device-specific is the DMOutput that it's attached to.
            ...
            : base(key, name, device)
        {
            ...
            AddToFeedbackList(VideoOutputResolutionFeedback, EdidManufacturerFeedback, EdidSerialNumberFeedback,
                EdidNameFeedback, EdidPreferredTimingFeedback);
            ...
            EiscApiAdvanced bridge)
        {
            ...
            if (rmc.VideoOutputResolutionFeedback != null)
                rmc.VideoOutputResolutionFeedback.LinkInputSig(
                    trilist.StringInput[joinMap.CurrentOutputResolution.JoinNumber]);
            ...
            if (rmc.EdidPreferredTimingFeedback != null)
                rmc.EdidPreferredTimingFeedback.LinkInputSig(
                    trilist.StringInput[joinMap.EdidPrefferedTiming.JoinNumber]);
            ...
            if (_rmc is DmRmcScalerC)
            {
                trilist.BooleanOutput[joinMap.BlankOutput.JoinNumber]
                    .SetBoolSigAction((b) => ((DmRmcScalerC)_rmc).HdmiOutput.BlankEnabled());
                trilist.BooleanOutput[joinMap.UnblankOutput.JoinNumber]
                    .SetBoolSigAction((b) => ((DmRmcScalerC)_rmc).HdmiOutput.BlankDisabled());
            ...
            trilist.SetUShortSigAction(joinMap.AudioVideoSource.JoinNumber,
                a => routing.ExecuteNumericSwitch(a, 1, eRoutingSignalType.AudioVideo));
        }
    ...
    {
        private static readonly Dictionary<string, Func<string, string, uint, CrestronGenericBaseDevice>>
            ProcessorFactoryDict;

        private static readonly Dictionary<string, Func<string, string, DMOutput, CrestronGenericBaseDevice>>
            ChassisCpu3Dict;
        ...
        static DmRmcHelper()
        {
            ProcessorFactoryDict = new Dictionary<string, Func<string, string, uint, CrestronGenericBaseDevice>>
            {
                ...
                {
                    "dmrmc4kz100c",
                    (k, n, i) => new DmRmc4kZ100CController(k, n, new DmRmc4kz100C(i, Global.ControlSystem))
                },
                ...
                {
                    "dmrmcscalerc",
                    (k, n, i) => new DmRmcScalerCController(k, n, new DmRmcScalerC(i, Global.ControlSystem))
                },
                {
                    "dmrmcscalers",
                    (k, n, i) => new DmRmcScalerSController(k, n, new DmRmcScalerS(i, Global.ControlSystem))
                },
                {
                    "dmrmcscalers2",
        ...
        /// <returns></returns>
        public static CrestronGenericBaseDevice GetDmRmcController(string key, string name, string typeName,
            DmRmcPropertiesConfig props)
        {
            typeName = typeName.ToLower();
            uint ipid = props.Control.IpIdInt;

            string pKey = props.ParentDeviceKey.ToLower();

            // Non-DM-chassis endpoints
            return pKey == "processor"
                ? GetDmRmcControllerForProcessor(key, name, typeName, ipid)
                : GetDmRmcControllerForChassis(key, name, typeName, props, pKey, ipid);
        }
        ...
        {
            IKeyed parentDev = DeviceManager.GetDeviceForKey(pKey);
        ...
        private static CrestronGenericBaseDevice GetDmRmcControllerForCpu3Chassis(string key, string name,
            string typeName,
            ...
            uint ipid, DmpsRoutingController controller, uint num)
        {
            ...
            Debug.Console(0, Debug.ErrorLogLevel.Error,
                "Cannot create DM-RMC of type '{0}' to output {1} on DMPS chassis", typeName, num);
            return null;
        }
        ...
        private static CrestronGenericBaseDevice GetDmRmcControllerForProcessor(string key, string name,
            string typeName, uint ipid)
        {
            try
            {
    ...
    public class DmRmcControllerFactory : EssentialsDeviceFactory<DmRmcControllerBase>
    {
        public DmRmcControllerFactory()
        {
            TypeNames = new List<string>
            {
                ...
                "dmrmc200c", "dmrmc200s", "dmrmc200s2", "dmrmcscalerc", "dmrmcscalers", "dmrmcscalers2",
                "dmrmc4kscalerc", "dmrmc4kscalercdsp",
        ...
        public override EssentialsDevice BuildDevice(DeviceConfig dc)
        {
            ...
            DmRmcPropertiesConfig props = JsonConvert.DeserializeObject
